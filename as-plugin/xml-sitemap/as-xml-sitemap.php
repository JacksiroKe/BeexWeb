...
class as_xml_sitemap
{
	...
	{
		switch ($option) {
			case 'xml_sitemap_show_articles':
	...
	public function admin_form()
	{
		...
		if (as_clicked('xml_sitemap_save_button')) {
			as_opt('xml_sitemap_show_articles', (int)as_post_text('xml_sitemap_show_articles_field'));
		...
		$form = array(
			...
			'fields' => array(
				'articles' => array(
					'label' => 'Include article pages',
					'type' => 'checkbox',
					'value' => (int)as_opt('xml_sitemap_show_articles'),
					'tags' => 'name="xml_sitemap_show_articles_field"',
				),
			),
		...
		if (as_using_tags()) {
			$form['fields']['tagqs'] = array(
				'label' => 'Include article list for each tag',
				'type' => 'checkbox',
		...
		if (as_using_categories()) {
			$form['fields']['categoryqs'] = array(
				'label' => 'Include article list for each category',
				'type' => 'checkbox',
	...
	public function process_request($request)
	{
		...
		if (as_opt('xml_sitemap_show_articles')) {
			$hotstats = as_db_read_one_assoc(as_db_query_sub(
				"SELECT MIN(hotness) AS base, MAX(hotness)-MIN(hotness) AS spread FROM ^posts WHERE type='P'"
			));

			$nextpostid = 0;

			while (1) {
				$articles = as_db_read_all_assoc(as_db_query_sub(
					"SELECT postid, title, hotness FROM ^posts WHERE postid>=# AND type='P' ORDER BY postid LIMIT 100",
					$nextpostid
				));
				...
				foreach ($articles as $article) {
					$this->sitemap_output(as_q_request($article['postid'], $article['title']),
						0.1 + 0.9 * ($article['hotness'] - $hotstats['base']) / (1 + $hotstats['spread']));
					$nextpostid = max($nextpostid, $article['postid'] + 1);
				}
			}
		}
		...
		// Article list for each category
		...
		if (as_using_categories() && as_opt('xml_sitemap_show_category_qs')) {
			...
			while (1) {
				$categories = as_db_read_all_assoc(as_db_query_sub(
					"SELECT categoryid, backpath FROM ^categories WHERE categoryid>=# AND pcount>0 ORDER BY categoryid LIMIT 2",
					$nextcategoryid
				));
				...
				foreach ($categories as $category) {
					$this->sitemap_output('articles/' . implode('/', array_reverse(explode('/', $category['backpath']))), 0.5);
