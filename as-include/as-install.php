<?php
/*
	Beex by Jackson Siro
	https://www.github.com/Jacksiro/BeexWeb/
	...
	More about this license: https://www.github.com/Jacksiro/BeexWeb/license.php
*/
...
if (!isset($pass_failure_type) && as_clicked('super')) {
	...
	if (as_db_count_users() == 0) { // prevent creating multiple accounts
		...
		$ingender = as_post_text('as_gender');
		$incountry = 'Kenya';
		$inmobile = as_post_text('as_mobile');
...
if (isset($pass_failure_type)) {
	...
	switch ($pass_failure_type) {
		...
		case 'select':
			...
			$errorhtml .= 'Could not switch to the Beex database. Please check the database name in the config file, and if necessary create the database in MySQL and grant appropriate user privileges.';
			break;
		...
		case 'query':
			...
			if (@$pass_failure_from_install) {
				...
				$errorhtml .= "Beex was unable to perform the installation query below. Please check the user in the config file has CREATE and ALTER permissions:\n\n".as_html($pass_failure_query."\n\nError ".$pass_failure_errno.": ".$pass_failure_error."\n\n");
			}
			else {
				$pgtitle = 'A Database Query Failed';
				$errorhtml .= "An Beex database query failed when generating this page.\n\nA full description of the failure is available in the web server's error log file.";
			}
			break;
	}
}
	...
	if (as_clicked('create')) {
		...
		$pgtitle = 'Your Beex database has been created';
		...
		if (AS_FINAL_EXTERNAL_USERS) {
			if (defined('AS_FINAL_WORDPRESS_INTEGRATE_PATH')) {
				...
				$success .= 'Your Beex database has been created and integrated with your WordPress site.';
			...
			elseif (defined('AS_FINAL_JOOMLA_INTEGRATE_PATH')) {
				...
				$success .= 'Your Beex database has been created and integrated with your Joomla! site.';
			}
			else {
				$success .= 'Your Beex database has been created for external user identity management. Please read the online documentation to complete integration.';
			}
		}
		else {
			$success .= 'Your Beex database has been created.';
		}
	}
	...
	if (as_clicked('nonuser')) {
		...
		$success .= 'The additional Beex database tables have been created.';
	}

	if (as_clicked('upgrade')) {
		as_db_upgrade_tables();
		$pgtitle = 'Your Beex database has been updated';
		$success .= 'Your Beex database has been updated.';
	}

	if (as_clicked('repair')) {
		as_db_install_tables();
		$pgtitle = 'Your Beex database has been repaired';
		$success .= 'The Beex database tables have been repaired.';
	}
	...
	switch ($check) {
		case 'none':
			if (@$pass_failure_errno == 1146) // don't show error if we're in installation process
				$errorhtml = '';
			$pgtitle = 'Welcome to BeExpress';
			$errorhtml .= 'Welcome to the BeExpress System (BEEX). It\'s time to set up your database!';

			if (AS_FINAL_EXTERNAL_USERS) {
				if (defined('AS_FINAL_WORDPRESS_INTEGRATE_PATH')) {
					$errorhtml .= "\n\nWhen you click below, your Beex site will be set up to integrate with the users of your WordPress site <a href=\"".as_html(get_option('home'))."\" target=\"_blank\">".as_html(get_option('blogname'))."</a>. Please consult the online documentation for more information.";
				}
				elseif (defined('AS_FINAL_JOOMLA_INTEGRATE_PATH')) {
					$jconfig = new JConfig();
					$errorhtml .= "\n\nWhen you click below, your Beex site will be set up to integrate with the users of your Joomla! site <a href=\"../\" target=\"_blank\">".$jconfig->sitename."</a>. It's also recommended to install the Joomla BEIntegration plugin for additional user-access control. Please consult the online documentation for more information.";
				}
				else {
					$errorhtml .= "\n\nWhen you click below, your Beex site will be set up to integrate with your existing user database and management. Members will be referenced with database column type ".as_html(as_get_mysql_user_column_type()).". Please consult the online documentation for more information.";
				}

				$buttons = array('create' => 'Set up the Database');
			}
			else {
				$errorhtml .= "\n\nWhen you click below, your Beex database will be set up to manage user identities and signins internally.\n\nIf you want to offer a single sign-on for an existing user base or website, please consult the online documentation before proceeding.";
				$buttons = array('create' => 'Set up the Database including User Management');
			}
			break;

		case 'old-version':
			$pgtitle = 'Need to Upgrade Beex Database';
			// don't show error if we need to upgrade
			if (!@$pass_failure_from_install)
				$errorhtml = '';

			// don't show error before this
			$errorhtml .= 'Your Beex database needs to be upgraded for this version of the software.';
			$buttons = array('upgrade' => 'Upgrade the Database');
			break;

		case 'non-users-missing':		
			$pgtitle = 'Non-Members Missing on your Beex';
			$errorhtml = 'This Beex site is sharing its users with another APS site, but it needs some additional database tables for its own content. Please click below to create them.';
			$buttons = array('nonuser' => 'Set up the Tables');
			break;

		case 'table-missing':
			$pgtitle = 'Your Beex Database is missing some Tables';
			$errorhtml .= 'One or more tables are missing from your Beex database.';
			$buttons = array('repair' => 'Repair the Database');
			break;

		case 'column-missing':
			$pgtitle = 'Database Missing Columns';
			$errorhtml .= 'One or more Beex database tables are missing a column.';
			$buttons = array('repair' => 'Repair the Database');
			break;
...
if (empty($errorhtml)) {
	if (empty($success)) {
		...
		$success = 'Your Beex database has been checked with no problems.';
	}
...
?>
		...
		<div class="content-section-footer inner_other">
			<center>
				<p>Copyright &copy; BeExpress by AppSmata Sol. <?php echo date('Y') ?></p>
			</center>
		</div>
	</body>
</html>
