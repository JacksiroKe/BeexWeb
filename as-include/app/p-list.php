...
/*
	...
	Description: Controller for most item listing pages, plus custom pages and plugin pages
...
/**
 * Returns the $as_content structure for a item list page showing $items retrieved from the
 * database. If $pagesize is not null, it sets the max number of items to display. If $count is
 * not null, pagination is determined by $start and $count. The page title is $sometitle unless
 * there are no items shown, in which case it's $nonetitle. $navcategories should contain the
 ...
 * shown, with per-category item counts if $categorypcount is true. The nav links will have the
 ...
 * @return array
 */
function as_p_list_page_content($items, $pagesize, $start, $count, $sometitle, $nonetitle,
	...
	$pagelinkparams = null, $categoryparams = null, $dummy = null)
{
	...
	if (!empty($items)) {
		...
		foreach ($items as $item) {
			$fields = as_any_to_q_html_fields($item, $userid, as_cookie_get(), $usershtml, null, as_post_html_options($item, $defaults));

			if (as_post_is_closed($item)) {
				$fields['closed'] = array(
...
/**
 * Return the sub navigation structure common to item listing pages
 * @param $sort
 * @param $categoryslugs
 * @return array
 */
function as_qs_sub_navigation($sort, $categoryslugs)
{
	...
	$navigation = array(
		...
		'likes' => array(
			'label' => as_lang('main/nav_most_likes'),
			'url' => as_path_html($request, array('sort' => 'likes')),
		),
