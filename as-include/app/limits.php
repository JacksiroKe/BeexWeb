...
function as_limits_calc_remaining($action, $userlimits, $iplimits)
{
	switch ($action) {
		...
		case AS_LIMIT_VOTES:
			$usermax = as_opt('max_rate_user_likes');
			$ipmax = as_opt('max_rate_ip_likes');
			break;
 ...
 * @param int $commentid
 */
function as_report_write_action($userid, $cookieid, $action, $articleid, $reviewid, $commentid)
{
}
