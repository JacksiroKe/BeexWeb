...
/*
	...
	^options (title=cache_*): cached values for various things (e.g. counting items)

	Recalculated in dorecountposts:
	==============================
	^posts (positivelikes, negativelikes, netlikes, hotness, rcount, rmaxlikes, flagcount): number of likes, hotness, reviews, review likes, flags
	...
	===================================
	^posts (categoryid): assign to reviews and comments based on their antecedent item
	^posts (catidpath1, catidpath2, catidpath3): hierarchical path to category ids (requires AS_CATEGORY_DEPTH=4)
	^categories (pcount): number of (visible) items in each category
...
function as_recalc_perform_step(&$state)
{
	...
	switch ($operation) {
		...
		case 'doreindexcontent_postreindex':
			...
			if (count($posts)) {
				...
				foreach ($posts as $postid => $post) {
					...
					as_post_index($postid, $post['type'], $post['articleid'], $post['parentid'], $post['title'], $post['content'],
		...
		case 'dorecountposts_postcount':
			...
			as_recalc_transition($state, 'dorecountposts_likecount');
			break;

		case 'dorecountposts_likecount':
			$postids = as_db_posts_get_for_recounting($next, 1000);

			if (count($postids)) {
				$lastpostid = max($postids);

				as_db_posts_likes_recount($next, $lastpostid);
				...
				as_recalc_transition($state, 'dorecountposts_rcount');
			}
			break;

		case 'dorecountposts_rcount':
			$postids = as_db_posts_get_for_recounting($next, 1000);
		...
		case 'dorefillevents':
			as_recalc_transition($state, 'dorefillevents_pcount');
			break;

		case 'dorefillevents_pcount':
			as_db_pcount_update();
			as_recalc_transition($state, 'dorefillevents_refill');
			break;

		case 'dorefillevents_refill':
			$articleids = as_db_qs_get_for_event_refilling($next, 1);
			...
			if (count($articleids)) {
				...
				$lastarticleid = max($articleids);

				foreach ($articleids as $articleid) {
					// Retrieve all posts relating to this item

					list($item, $childposts, $achildposts) = as_db_select_with_pending(
						as_db_full_post_selectspec(null, $articleid),
					...
					foreach ($posts as $postid => $post) {
						$followonq = ($post['basetype'] == 'P') && ($postid != $articleid);

						if ($followonq) {
							$updatetype = AS_UPDATE_FOLLOWS;
						} elseif ($post['basetype'] == 'C' && @$posts[$post['parentid']]['basetype'] == 'P') {
						...
						as_create_event_for_q_user($articleid, $postid, $updatetype, $post['userid'], @$posts[$post['parentid']]['userid'], $post['created']);

						if (isset($post['updated']) && !$followonq) {
							as_create_event_for_q_user($articleid, $postid, $post['updatetype'], $post['lastuserid'], $post['userid'], $post['updated']);
						}
					}

					// Tags and categories of item

					as_create_event_for_tags($item['tags'], $articleid, null, $item['userid'], $item['created']);
					as_create_event_for_category($item['categoryid'], $articleid, null, $item['userid'], $item['created']);
						...
						foreach ($comments as $comment) {
							foreach ($keyuserids as $keyuserid => $dummy) {
								if ($keyuserid != $comment['userid'] && $keyuserid != @$posts[$parentid]['userid']) {
									as_db_event_create_not_entity($keyuserid, $articleid, $comment['postid'], AS_UPDATE_FOLLOWS, $comment['userid'], $comment['created']);
								}
							}
				...
				$next = 1 + $lastarticleid;
				$done += count($articleids);
				$continue = true;
		...
		case 'dorecalccategories_recount':
			...
			if (count($categoryids)) {
				...
				foreach ($categoryids as $categoryid) {
					as_db_ifcategory_pcount_update($categoryid);
				}
		...
		case 'dodeletehidden_reviews':
			...
			if (count($posts)) {
				...
				as_recalc_transition($state, 'dodeletehidden_articles');
			}
			break;

		case 'dodeletehidden_articles':
			$posts = as_db_posts_get_for_deleting('P', $next, 1);
...
function as_recalc_stage_length($operation)
{
	switch ($operation) {
		...
		case 'doreindexcontent_postreindex':
			$length = as_opt('cache_pcount') + as_opt('cache_rcount') + as_opt('cache_ccount');
			break;
		...
		case 'dorefillevents_refill':
			$length = as_opt('cache_pcount') + as_db_count_posts('P_HIDDEN');
			break;
		...
		case 'dodeletehidden_articles':
			$length = count(as_db_posts_get_for_deleting('P'));
			break;
...
function as_recalc_get_message($state)
{
	...
	switch ($operation) {
		...
		case 'dorecountposts_likecount':
			$message = as_recalc_progress_lang('admin/recount_posts_likes_recounted', $done, $length);
			break;
		...
		case 'dodeletehidden_articles':
			$message = as_recalc_progress_lang('admin/hidden_articles_deleted', $done, $length);
			break;
