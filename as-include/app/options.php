...
function as_default_option($name)
{
	...
	$fixed_defaults = array(
		...
		'allow_close_articles' => 1,
		'allow_close_own_articles' => 1,
		...
		'hot_weight_likes' => 100,
		'links_in_new_window' => 1,
		//'logo_height' => 150,
		//'logo_show' => 1,
		//'logo_show' => 1,
		//'logo_width' => 150,
		//'logo_url' => 'as-media/as-logo.png',
		'mailing_per_minute' => 500,
		'match_write_check_qs' => 3,
		...
		'page_size_write_check_qs' => 5,
		'page_size_write_tags' => 5,
		'page_size_home' => 20,
		'page_size_hot_qs' => 20,
		'page_size_pms' => 10,
		'page_size_q_as' => 10,
		...
		'permit_view_likers_flaggers' => AS_PERMIT_ADMINS,
		'permit_like_a' => AS_PERMIT_USERS,
		'permit_like_c' => AS_PERMIT_USERS,
		'permit_like_down' => AS_PERMIT_USERS,
		'permit_like_q' => AS_PERMIT_USERS,
		'points_a_selected' => 30,
		'points_a_liked_max_gain' => 20,
		'points_a_liked_max_loss' => 5,
		'points_base' => 100,
		'points_c_liked_max_gain' => 10,
		'points_c_liked_max_loss' => 3,
		'points_multiple' => 10,
		'points_per_c_liked_down' => 0,
		'points_per_c_liked_up' => 0,
		'points_post_a' => 4,
		'points_post_q' => 2,
		'points_q_liked_max_gain' => 10,
		'points_q_liked_max_loss' => 3,
		'points_select_a' => 3,
	...
	switch ($name) {
		...
		case 'permit_post_q': // convert from deprecated option if available
			$value = as_opt('write_needs_signin') ? AS_PERMIT_USERS : AS_PERMIT_ALL;
			break;
		...
		case 'points_like_up_q':
		case 'points_like_down_q':
			$oldvalue = as_opt('points_like_on_q');
			$value = is_numeric($oldvalue) ? $oldvalue : 1;
			break;

		case 'points_like_up_a':
		case 'points_like_down_a':
			$oldvalue = as_opt('points_like_on_a');
			$value = is_numeric($oldvalue) ? $oldvalue : 1;
			break;

		case 'points_per_q_liked_up':
		case 'points_per_q_liked_down':
			$oldvalue = as_opt('points_per_q_liked');
			$value = is_numeric($oldvalue) ? $oldvalue : 1;
			break;

		case 'points_per_a_liked_up':
		case 'points_per_a_liked_down':
			$oldvalue = as_opt('points_per_a_liked');
...
/**
 * Return an array of defaults for the $options parameter passed to as_post_html_fields() and its ilk for posts of $basetype='P'/'R'/'C'
 * Set $full to true if these posts will be viewed in full, i.e. on a item page rather than a item listing
 * @param $basetype
 * @param bool $full
 * @return array|mixed
 */
function as_post_html_defaults($basetype, $full = false)
{
	...
	return array(
		'categoryview' => $basetype == 'P' && as_using_categories(),
		'contentview' => $full,
		'likeview' => as_get_like_view($basetype, $full),
		'flagsview' => as_opt('flagging_of_posts') && $full,
		'favoritedview' => true,
		'reviewsview' => $basetype == 'P',
		'viewsview' => $basetype == 'P' && as_opt('do_count_q_views') && ($full ? as_opt('show_view_count_q_page') : as_opt('show_view_counts')),
		'whatview' => true,
		'whatlink' => as_opt('show_a_c_links'),
		'whenview' => as_opt('show_when_created'),
		'ipview' => !as_user_permit_error('permit_anon_view_ips'),
		'whoview' => true,
		'avatarsize' => as_opt('avatar_p_list_size'),
 ...
 * list. Set $full to true if these posts will be viewed in full, i.e. on a item page rather than a item listing.
 * @param $post
 * @param $defaults
 * @param bool $full
...
function as_post_html_options($post, $defaults = null, $full = false)
{
	...
	$defaults['likeview'] = as_get_like_view($post, $full);
...
/**
 * Return $likeview parameter to pass to as_post_html_fields() in /as-include/app/format.php.
 * @param array|string $postorbasetype The post, or for compatibility just a basetype, i.e. 'P', 'R' or 'C'
 ...
 */
function as_get_like_view($postorbasetype, $full = false, $enabledif = true)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

	// The 'level' and 'approve' permission errors are taken care of by disabling the voting buttons.
	// Others are reported to the user after they click, in as_like_error_html(...)
	...
	{
		case 'P':
			$view = as_opt('voting_on_qs');
			$permitOpt = 'permit_like_q';
			break;
		case 'R':
			$view = as_opt('voting_on_as');
			$permitOpt = 'permit_like_a';
			break;
		case 'C':
			$view = as_opt('voting_on_cs');
			$permitOpt = 'permit_like_c';
			break;
		default:
			$view = false;
			break;
	...
	if (!$enabledif || ($basetype == 'P' && !$full && as_opt('voting_on_q_page_only'))) {
		...
		elseif ($permiterror == 'approve')
			...
			$permiterrordown = isset($post) ? as_user_post_permit_error('permit_like_down', $post) : as_user_permit_error('permit_like_down');
	...
	return (as_opt('likes_separated') ? 'updown' : 'net') . $disabledsuffix;
}
...
/**
 * Return whether the option is set to classify items by tags
 */
...
/**
 * Return whether the option is set to classify items by categories
 */
...
function as_get_permit_options()
{
	...
	if (as_opt('voting_on_qs'))
		$permits[] = 'permit_like_q';

	if (as_opt('voting_on_as'))
		$permits[] = 'permit_like_a';

	if (as_opt('voting_on_cs'))
		$permits[] = 'permit_like_c';

	if (as_opt('voting_on_qs') || as_opt('voting_on_as') || as_opt('voting_on_cs'))
		$permits[] = 'permit_like_down';
	...
	if (as_opt('allow_close_articles'))
		$permits[] = 'permit_close_q';

	array_push($permits, 'permit_select_a', 'permit_anon_view_ips');

	if (as_opt('voting_on_qs') || as_opt('voting_on_as') || as_opt('voting_on_cs') || as_opt('flagging_of_posts'))
		$permits[] = 'permit_view_likers_flaggers';
