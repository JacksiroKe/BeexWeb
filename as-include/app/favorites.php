 ...
 * @param string $entityid ID of the entity being favorited (e.g. postid for items)
...
/**
 * Returns content to set in $as_content['p_list'] for a user's favorite $items. Pre-generated
 * user HTML in $usershtml.
 * @param $items
 * @param $usershtml
 * @return array
 */
function as_favorite_p_list_view($items, $usershtml)
{
	$p_list = array(
		'ps' => array(),
	);
	...
	foreach ($items as $item) {
		$p_list['ps'][] = as_post_html_fields($item, as_get_logged_in_userid(), as_cookie_get(),
			$usershtml, null, as_post_html_options($item, $defaults));
	}
...
function as_favorite_categories_view($categories)
{
	...
	foreach ($categories as $category) {
		$cat_url = as_path_html('items/' . implode('/', array_reverse(explode('/', $category['backpath']))));
		$cat_anchor = $category['pcount'] == 1
			? as_lang_html_sub('main/1_article', '1', '1')
			: as_lang_html_sub('main/x_articles', as_format_number($category['pcount'], 0, true));
