...
define('AS_FIELD_FLAGS_ON_SIGNUP', 4);
...
if (AS_FINAL_EXTERNAL_USERS) {
	 ...
	 * @return mixed
	 */
	function as_set_signed_in_user($userid, $handle = '', $remember = false, $source = null)
	{
		if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	function as_log_in_external_user($source, $identifier, $fields)
	{
		...
		if ($countusers) // user exists so log them in
			as_set_signed_in_user($users[0]['userid'], $users[0]['handle'], false, $source);
			...
			if (count($users) == 1) {
				...
				as_set_signed_in_user($userid, $handle, false, $source);
			}
		}
	}
	...
	function as_get_logged_in_profiles()
	{
		return as_get_logged_in_user_field('profiles');
	}
	/**
	 ...
	 */
	function as_get_user_avatar_html($flags, $email, $handle, $blobId, $width, $height, $size, $class = '', $padding = false)
	{
		if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
		...
		switch ($avatarSource) {
			case 'gravatar':
				...
				break;
			case 'local-user':
				$html = as_get_avatar_blob_html($blobId, $width, $height, $size, $class, $padding);
				break;
			case 'local-default':
				$html = as_get_avatar_blob_html(as_opt('avatar_default_blobid'), as_opt('avatar_default_width'), as_opt('avatar_default_height'), $size, $class, $padding);
				if (strlen($handle) == 0) {
					return $html;
				}
				break;
...
function as_user_type($type, $full = false)
{
	if ($full) {
		switch ($type){
			case 'SA': 	return 'Super-Admin';
			case 'A': 	return 'Admin';
			case 'ENR': return 'Employee (ENR)';
			case 'ER': 	return 'Employee (ER)';
			case 'SSP': return 'Physical Outlet (SSP)';
			case 'MSP': return 'Mobi Shop (MSP)';
			case 'BS': 	return 'Supplier (BS)';
			case 'IS': 	return 'Supplier (IS)';
			case 'BC': 	return 'Customer (BC)';
			case 'IC': 	return 'Customer (IC)';
			default: 	return 'User';
		}
	}
	else return $type;
 ...
 * Return level of currently logged in user, or null if none
 */
function as_get_logged_in_type($full = false)
{
	$type = as_get_logged_in_user_field('usertype');
	if ($full) {
		switch ($type){
			case 'SA': 	return 'Super-Admin';
			case 'A': 	return 'Admin';
...
/**
 * Set or extend the cookie in browser of non signed-in users which identifies them for the purposes of form security (anti-CSRF protection)
 */
...
function as_check_form_security_code($action, $value)
{
	...
	if (!empty($reportproblems) && AS_DEBUG_PERFORMANCE) {
		@error_log(
			'PHP AppSmata form security violation for ' . $action .
