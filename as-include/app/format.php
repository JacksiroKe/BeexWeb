...
/**
 * Get an array listing all of the logged in user's favorite items, except their favorited items (these are excluded because
 * users tend to favorite many more items than other things.) The top-level array can contain three keys - 'user' for favorited
...
function as_post_html_fields($post, $userid, $cookieid, $usershtml, $dummy, $options = array())
{
	...
	$isorder = isset($post['isorder']) ? $post['isorder'] : null;
	...
	$fields['classes'] = ($isarticle && $favoritedview && @$post['userfavoriteq']) ? 'as-q-favorited' : '';
	if ($isarticle && as_post_is_closed($post)) {
	...
	// Article-specific stuff (title, URL, tags, review count, category)
	...
	if ($isorder) {
		...
		if (isset($post['title'])) {
			...
			$fields['title'] = as_html($post['title']).as_html($post['categoryname'] != $post['title'] ? ' '.$post['categoryname'] : '');
			if ($microdata) {
		...
		$fields['isorder'] = true;
		$fields['volume'] = $post['volume'];
		$fields['weight'] = (int)$post['quantity'] * (int)$post['weight'];
		$fields['buyprice'] = $post['buyprice'];
		$fields['saleprice'] = $post['saleprice'];
		$fields['totalprice'] = (int)$post['quantity'] * (int)$post['saleprice'];
		$fields['state'] = $post['state'];
		$fields['color'] = $post['color'];
		$fields['texture'] = $post['texture'];
		$fields['totalqty'] = $post['quantity'];
		$fields['quantity'] = $post['quantity'];
		$fields['stockin'] = $post['stockin'];
		$fields['stockout'] = $post['stockout'];
		$fields['customer'] = $post['orderedby'];
		$fields['address'] = $post['address'];
		
		$fields['images'] = $post['images'];
		$fields['icon'] = as_path('as-media/'.(strlen($post['icon']) ? $post['icon'] : 'appicon.png'));
		$fields['caticon'] = $post['caticon'];
		
		/*if (isset($post['summary'])) {
			$body = strip_tags($post['summary']);
			//$fields['content'] = substr($body, 0, strpos($body, ' ', as_opt('q_summary_length'))) . ' ... ' .
			$fields['content'] = substr($body, 0, strpos($body, ' ', 50)) . ' ... ' .
		...
		if (@$options['reviewsview'] && isset($post['rcount'])) {
			$fields['reviews_raw'] = $post['rcount'];

			$fields['reviews'] = ($post['rcount'] == 1) ? as_lang_html_sub_split('main/1_review', '1', '1')
				: as_lang_html_sub_split('main/x_reviews', as_format_number($post['rcount'], 0, true));
	...
	if ($isarticle) {
		...
		$fields['stockin'] = $post['stockin'];
		$fields['stockout'] = $post['stockout'];
		$fields['manufacturer'] = $post['manufacturer'];
	...
	if ($isreview) {
		...
		if ($isselected)
			$fields['select_text'] = as_lang_html('item/select_text');
	}
	...
	if (@$options['likeview']) {
		$likeview = $options['likeview'];

		// Calculate raw values and pass through

		if (@$options['olikeview'] && isset($post['opostid'])) {
			$positivelikes = (int)@$post['opositivelikes'];
			$negativelikes = (int)@$post['onegativelikes'];
			$fields['like_opostid'] = true; // for likers/flaggers layer
		} else {
			$positivelikes = (int)@$post['positivelikes'];
			$negativelikes = (int)@$post['negativelikes'];
		}

		$netlikes = $positivelikes - $negativelikes;

		$fields['positivelikes_raw'] = $positivelikes;
		$fields['negativelikes_raw'] = $negativelikes;
		$fields['netlikes_raw'] = $netlikes;

		// Create HTML versions...

		$positivelikeshtml = as_html(as_format_number($positivelikes, 0, true));
		$negativelikeshtml = as_html(as_format_number($negativelikes, 0, true));

		if ($netlikes >= 1)
			$netlikesPrefix = '+';
		elseif ($netlikes <= -1)
			$netlikesPrefix = '&ndash;';
		else
			$netlikesPrefix = '';

		$netlikes = abs($netlikes);
		$netlikeshtml = $netlikesPrefix . as_html(as_format_number($netlikes, 0, true));

		// Pass information on like viewing

		// $likeview will be one of:
		...
		$fields['like_view'] = (substr($likeview, 0, 6) == 'updown') ? 'updown' : 'net';

		$fields['like_on_page'] = strpos($likeview, '-disabled-page') ? 'disabled' : 'enabled';

		if ($iscomment) {
			// for comments just show number, no additional text
			$fields['positivelikes_view'] = array('prefix' => '', 'data' => $positivelikeshtml, 'suffix' => '');
			$fields['negativelikes_view'] = array('prefix' => '', 'data' => $negativelikeshtml, 'suffix' => '');
			$fields['netlikes_view'] = array('prefix' => '', 'data' => $netlikeshtml, 'suffix' => '');
		} else {
			$fields['positivelikes_view'] = $positivelikes == 1
				? as_lang_html_sub_split('main/1_liked', $positivelikeshtml, '1')
				: as_lang_html_sub_split('main/x_liked', $positivelikeshtml);
			$fields['negativelikes_view'] = $negativelikes == 1
				? as_lang_html_sub_split('main/1_disliked', $negativelikeshtml, '1')
				: as_lang_html_sub_split('main/x_disliked', $negativelikeshtml);
			$fields['netlikes_view'] = $netlikes == 1
				? as_lang_html_sub_split('main/1_like', $netlikeshtml, '1')
				: as_lang_html_sub_split('main/x_likes', $netlikeshtml);
		}

		// schema.org microdata - like display might be formatted (e.g. '2k') so we use meta tag for true count
		if ($microdata) {
			$fields['netlikes_view']['suffix'] .= ' <meta itemprop="positivelikeCount" content="' . as_html($netlikes) . '"/>';
			$fields['positivelikes_view']['suffix'] .= ' <meta itemprop="positivelikeCount" content="' . as_html($positivelikes) . '"/>';
		}

		// Voting buttons

		$fields['like_tags'] = 'id="voting_' . as_html($postid) . '"';
		$onclick = 'onclick="return as_like_click(this);"';

		if ($fields['hidden']) {
			$fields['like_state'] = 'disabled';
			$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_disabled_hidden_post') . '"';
			$fields['like_down_tags'] = $fields['like_up_tags'];

		} elseif ($fields['queued']) {
			$fields['like_state'] = 'disabled';
			$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_disabled_queued') . '"';
			$fields['like_down_tags'] = $fields['like_up_tags'];

		} elseif ($isbyuser) {
			$fields['like_state'] = 'disabled';
			$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_disabled_my_post') . '"';
			$fields['like_down_tags'] = $fields['like_up_tags'];

		} elseif (strpos($likeview, '-disabled-')) {
			$fields['like_state'] = (@$post['userlike'] > 0) ? 'liked_up_disabled' : ((@$post['userlike'] < 0) ? 'liked_down_disabled' : 'disabled');

			if (strpos($likeview, '-disabled-page'))
				$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_disabled_q_page_only') . '"';
			elseif (strpos($likeview, '-disabled-approve'))
				$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_disabled_approve') . '"';
			else
				$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_disabled_level') . '"';

			$fields['like_down_tags'] = $fields['like_up_tags'];

		} elseif (@$post['userlike'] > 0) {
			$fields['like_state'] = 'liked_up';
			$fields['like_up_tags'] = 'title="' . as_lang_html('main/liked_up_popup') . '" name="' . as_html('like_' . $postid . '_0_' . $elementid) . '" ' . $onclick;
			$fields['like_down_tags'] = ' ';

		} elseif (@$post['userlike'] < 0) {
			$fields['like_state'] = 'liked_down';
			$fields['like_up_tags'] = ' ';
			$fields['like_down_tags'] = 'title="' . as_lang_html('main/liked_down_popup') . '" name="' . as_html('like_' . $postid . '_0_' . $elementid) . '" ' . $onclick;

		} else {
			$fields['like_up_tags'] = 'title="' . as_lang_html('main/like_up_popup') . '" name="' . as_html('like_' . $postid . '_1_' . $elementid) . '" ' . $onclick;

			if (strpos($likeview, '-uponly-level')) {
				$fields['like_state'] = 'up_only';
				$fields['like_down_tags'] = 'title="' . as_lang_html('main/like_disabled_down') . '"';

			} elseif (strpos($likeview, '-uponly-approve')) {
				$fields['like_state'] = 'up_only';
				$fields['like_down_tags'] = 'title="' . as_lang_html('main/like_disabled_down_approve') . '"';

			} else {
				$fields['like_state'] = 'enabled';
				$fields['like_down_tags'] = 'title="' . as_lang_html('main/like_down_popup') . '" name="' . as_html('like_' . $postid . '_-1_' . $elementid) . '" ' . $onclick;
			}
		}
	}
	...
	if (@$options['whatview']) {
		$fields['what'] = as_lang_html($isarticle ? 'main/posted' : ($isreview ? 'main/reviewed' : 'main/commented'));

		if (@$options['whatlink'] && strlen(@$options['q_request'])) {
			$fields['what_url'] = $post['basetype'] == 'P'
 ...
 * $item, as retrieved from database, with fields prefixed 'o' for the review, comment or edit.
 * $userid, $cookieid, $usershtml, $options are passed through to as_post_html_fields(). If $item['opersonal']
 ...
 * @return array
 */
function as_other_to_q_html_fields($item, $userid, $cookieid, $usershtml, $dummy, $options)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

	require_once AS_INCLUDE_DIR . 'app/updates.php';

	$fields = as_post_html_fields($item, $userid, $cookieid, $usershtml, null, $options);
	...
	switch ($item['obasetype'] . '-' . @$item['oupdatetype']) {
		...
		case 'Q-' . AS_UPDATE_VISIBLE:
			if (@$item['opersonal'])
				$langstring = $item['hidden'] ? 'misc/your_q_hidden' : 'misc/your_q_reshown';
			else
				$langstring = $item['hidden'] ? 'main/hidden' : 'main/reshown';
			break;

		case 'Q-' . AS_UPDATE_CLOSED:
			$isClosed = as_post_is_closed($item);
			if (@$item['opersonal'])
		...
		case 'Q-' . AS_UPDATE_TAGS:
			$langstring = @$item['opersonal'] ? 'misc/your_q_retagged' : 'main/retagged';
			break;

		case 'Q-' . AS_UPDATE_CATEGORY:
			$langstring = @$item['opersonal'] ? 'misc/your_q_recategorized' : 'main/recategorized';
			break;

		case 'A-':
			$langstring = @$item['opersonal'] ? 'misc/your_q_reviewed' : 'main/reviewed';
			break;

		case 'A-' . AS_UPDATE_SELECTED:
			$langstring = @$item['opersonal'] ? 'misc/your_a_selected' : 'main/review_selected';
			break;

		case 'A-' . AS_UPDATE_VISIBLE:
			if (@$item['opersonal'])
				$langstring = $item['ohidden'] ? 'misc/your_a_hidden' : 'misc/your_a_reshown';
			else
				$langstring = $item['ohidden'] ? 'main/hidden' : 'main/review_reshown';
			break;

		case 'A-' . AS_UPDATE_CONTENT:
			$langstring = @$item['opersonal'] ? 'misc/your_a_edited' : 'main/review_edited';
			break;

		case 'Q-' . AS_UPDATE_FOLLOWS:
			$langstring = @$item['opersonal'] ? 'misc/your_a_articleed' : 'main/posted_related_q';
			break;
		...
		case 'C-' . AS_UPDATE_C_FOR_Q:
			$langstring = @$item['opersonal'] ? 'misc/your_q_commented' : 'main/commented';
			break;

		case 'C-' . AS_UPDATE_C_FOR_A:
			$langstring = @$item['opersonal'] ? 'misc/your_a_commented' : 'main/commented';
			break;

		case 'C-' . AS_UPDATE_FOLLOWS:
			$langstring = @$item['opersonal'] ? 'misc/your_c_followed' : 'main/commented';
			break;

		case 'C-' . AS_UPDATE_TYPE:
			$langstring = @$item['opersonal'] ? 'misc/your_c_moved' : 'main/comment_moved';
			break;

		case 'C-' . AS_UPDATE_VISIBLE:
			if (@$item['opersonal'])
				$langstring = $item['ohidden'] ? 'misc/your_c_hidden' : 'misc/your_c_reshown';
			else
				$langstring = $item['ohidden'] ? 'main/hidden' : 'main/comment_reshown';
			break;

		case 'C-' . AS_UPDATE_CONTENT:
			$langstring = @$item['opersonal'] ? 'misc/your_c_edited' : 'main/comment_edited';
			break;

		case 'Q-' . AS_UPDATE_CONTENT:
		default:
			$langstring = @$item['opersonal'] ? 'misc/your_q_edited' : 'main/edited';
			break;
	}
	...
	if ($item['obasetype'] != 'P' || @$item['oupdatetype'] == AS_UPDATE_FOLLOWS)
		$fields['what_url'] = as_q_path_html($item['postid'], $item['title'], false, $item['obasetype'], $item['opostid']);

	if (@$options['contentview'] && !empty($item['ocontent'])) {
		$viewer = as_load_viewer($item['ocontent'], $item['oformat']);

		$fields['content'] = $viewer->get_html($item['ocontent'], $item['oformat'], array(
			'blockwordspreg' => @$options['blockwordspreg'],
			'showurllinks' => @$options['showurllinks'],
			'linksnewwindow' => @$options['linksnewwindow'],
		));
	...
	if (@$options['whenview'])
		$fields['when'] = as_when_to_html($item['otime'], @$options['fulldatedays']);

	if (@$options['whoview']) {
		$isbyuser = as_post_is_by_user(array('userid' => $item['ouserid'], 'cookieid' => @$item['ocookieid']), $userid, $cookieid);

		$fields['who'] = as_who_to_html($isbyuser, $item['ouserid'], $usershtml, @$options['ipview'] ? @inet_ntop(@$item['oip']) : null, false, @$item['oname']);
		if (isset($item['opoints'])) {
			if (@$options['pointsview'])
				$fields['who']['points'] = ($item['opoints'] == 1) ? as_lang_html_sub_split('main/1_point', '1', '1')
					: as_lang_html_sub_split('main/x_points', as_format_number($item['opoints'], 0, true));

			if (isset($options['pointstitle']))
				$fields['who']['title'] = as_get_points_title_html($item['opoints'], $options['pointstitle']);
		}

		if (isset($item['olevel']))
			$fields['who']['level'] = as_html(as_user_level_string($item['olevel']));
	}

	unset($fields['flags']);
	if (@$options['flagsview'] && @$item['oflagcount']) {
		$fields['flags'] = ($item['oflagcount'] == 1) ? as_lang_html_sub_split('main/1_flag', '1', '1')
			: as_lang_html_sub_split('main/x_flags', $item['oflagcount']);
	}

	unset($fields['avatar']);
	if (@$options['avatarsize'] > 0) {
		if (AS_FINAL_EXTERNAL_USERS)
			$fields['avatar'] = as_get_external_avatar_html($item['ouserid'], $options['avatarsize'], false);
		else
			$fields['avatar'] = as_get_user_avatar_html($item['oflags'], $item['oemail'], $item['ohandle'],
				$item['oavatarblobid'], $item['oavatarwidth'], $item['oavatarheight'], $options['avatarsize']);
	}
...
/**
 * Based on the elements in $item, return HTML to be passed to theme layer to link
 * to the item, or to an associated review, comment or edit.
 * @param $item
 * @param $userid
 * @param $cookieid
 * @param $usershtml
 ...
 * @return array
 */
function as_any_to_q_html_fields($item, $userid, $cookieid, $usershtml, $dummy, $options)
{
	if (isset($item['opostid']))
		$fields = as_other_to_q_html_fields($item, $userid, $cookieid, $usershtml, null, $options);
	else
		$fields = as_post_html_fields($item, $userid, $cookieid, $usershtml, null, $options);
...
/**
 * Each element in $items represents a item and optional associated review, comment or edit, as retrieved from database.
 * Return it sorted by the date appropriate for each element, without removing duplicate references to the same item.
 * @param $items
 * @return mixed
 */
function as_any_sort_by_date($items)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

	require_once AS_INCLUDE_DIR . 'util/sort.php';

	foreach ($items as $key => $item) // collect information about action referenced by each $item
		$items[$key]['sort'] = -(isset($item['opostid']) ? $item['otime'] : $item['created']);

	as_sort_by($items, 'sort');
...
/**
 * Each element in $items represents a item and optional associated review, comment or edit, as retrieved from database.
 * Return it sorted by the date appropriate for each element, and keep only the first item related to each item.
 * @param $items
 * @return array
 */
function as_any_sort_and_dedupe($items)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	foreach ($items as $key => $item) { // collect information about action referenced by each $item
		if (isset($item['opostid'])) {
			$items[$key]['_time'] = $item['otime'];
			$items[$key]['_type'] = $item['obasetype'];
			$items[$key]['_userid'] = @$item['ouserid'];
		} else {
			$items[$key]['_time'] = $item['created'];
			$items[$key]['_type'] = 'P';
			$items[$key]['_userid'] = $item['userid'];
		}

		$items[$key]['sort'] = -$items[$key]['_time'];
	}
	as_sort_by($items, 'sort');

	$keeparticles = array(); // now remove duplicate references to same item
	foreach ($items as $item) { // going in order from most recent to oldest
		$laterarticle = @$keeparticles[$item['postid']];

		if (isset($laterarticle)) {
			// the two events were within 5 minutes of each other
			$close_events = abs($laterarticle['_time'] - $item['_time']) < 300;

			$later_edit =
				@$laterarticle['oupdatetype'] &&  // the more recent reference was an edit
				!@$item['oupdatetype'] &&  // this is not an edit
				$laterarticle['_type'] == $item['_type'] &&  // the same part (Q/A/C) is referenced here
				$laterarticle['_userid'] == $item['_userid'];  // the same user made the later edit

			// this item (in an update list) is personal to the user, but the other one was not
			$this_personal = @$item['opersonal'] && !@$laterarticle['opersonal'];

			if ($close_events && ($later_edit || $this_personal)) {
				// Remove any previous instance of the post to force a new position
				unset($keeparticles[$item['postid']]);
				$keeparticles[$item['postid']] = $item;
			}
 ...
 * @return array
 */
function as_any_get_userids_handles($items)
{
 ...
 * incorrect URLs from (http://www.appsmata.org) but allow URLs such as http://www.wikipedia.org/Computers_(Software)
 * @param $html
...
/**
 * Return HTML that suggests browsing all items (in the category specified by $categoryrequest, if
...
function as_html_suggest_qs_tags($usingtags = false, $categoryrequest = null)
{
	...
	return strtr(
		...
		array(
			'^1' => '<a href="' . as_path_html('items' . ($hascategory ? ('/' . $categoryrequest) : '')) . '">',
			'^2' => '</a>',
...
/**
 * Return HTML that suggest getting things started by writting an item, in $categoryid if not null
 * @param null $categoryid
 * @return mixed|string
 */
function as_html_suggest_write($categoryid = null)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	return strtr(
		...
		array(
			'^1' => '<a href="' . as_path_html('write', strlen($categoryid) ? array('cat' => $categoryid) : null) . '">',
			'^2' => '</a>',
		)
	);
}
 ...
 * and links beginning with $pathprefix, and showing item counts if $showpcount
 * @param $categories
 * @param null $selectedid
 * @param string $pathprefix
 * @param bool $showpcount
 * @param null $pathparams
 * @return array|mixed
 */
function as_category_navigation($categories, $selectedid = null, $pathprefix = '', $showpcount = true, $pathparams = null)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	return as_category_navigation_sub($parentcategories, null, $selecteds, $pathprefix, $showpcount, $pathparams, $favoritemap);
}
 ...
 * @return array|mixed
 */
function as_category_navigation_sub($parentcategories, $parentid, $selecteds, $pathprefix, $showpcount, $pathparams, $favoritemap = null)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	if (isset($parentcategories[$parentid])) {
		foreach ($parentcategories[$parentid] as $category) {
			$navigation[as_html($category['tags'])] = array(
				...
				'note' => $showpcount ? ('(' . as_html(as_format_number($category['pcount'], 0, true)) . ')') : null,
...
function as_user_sub_navigation($handle, $selected, $ismyuser = false)
{
	$navigation = array(
		...
		'items' => array(
			...
			'url' => as_path_html('user/' . $handle . '/items'),
		),

		'orders' => array(
			'label' => as_lang_html('misc/nav_user_os'),
			'url' => as_path_html('user/' . $handle . '/orders'),
		),
...
{ 
	...
	if (as_is_logged_in())
	{
		$hasavatar = as_get_user_avatar_html($user['flags'], $user['email'], $user['handle'], $user['avatarblobid'], 
...
function as_set_up_tag_field(&$as_content, &$field, $fieldname, $tags, $exampletags, $completetags, $maxtags)
{
	...
	$field['label'] = as_lang_html($separatorcomma ? 'item/i_tags_comma_label' : 'item/i_tags_label');
	...
	$field['note'] =
		'<span id="tag_examples_title"' . (count($exampletags) ? '' : $sdn) . '>' . as_lang_html('item/example_tags') . '</span>' .
		'<span id="tag_complete_title"' . $sdn . '>' . as_lang_html('item/matching_tags') . '</span><span id="tag_hints">';
...
function as_set_up_category_field(&$as_content, &$field, $fieldname, $navcategories, $categoryid, $allownone, $allownosub, $maxdepth = null, $excludecategoryid = null)
{
	...
	$field['note'] =
		...
		'<noscript style="color:red;">' . as_lang_html('item/category_js_note') . '</noscript>' .
		'</div>';
}
...
{
	$fields['name'] = array(
		'label' => as_lang_html('item/anon_name_label'),
 ...
 * $basetype is 'P', 'R' or 'C' for item, review or comment. $signin_email is the email of logged in user,
...
function as_set_up_notify_fields(&$as_content, &$fields, $basetype, $signin_email, $innotify, $inemail, $errors_email, $fieldprefix = '')
{
	...
	switch ($basetype) {
		case 'P':
			$labelwriteemail = as_lang_html('item/i_notify_email');
			$labelonly = as_lang_html('item/i_notify_label');
			$labelgotemail = as_lang_html('item/i_notify_x_label');
			break;

		case 'R':
			$labelwriteemail = as_lang_html('item/a_notify_email');
			$labelonly = as_lang_html('item/a_notify_label');
			$labelgotemail = as_lang_html('item/a_notify_x_label');
			break;

		case 'C':
			$labelwriteemail = as_lang_html('item/c_notify_email');
			$labelonly = as_lang_html('item/c_notify_label');
			$labelgotemail = as_lang_html('item/c_notify_x_label');
			break;
	}
	...
	if (empty($signin_email)) {
		...
		$fields['email'] = array(
			...
			'note' => as_lang_html('item/notify_email_note'),
...
function as_load_theme_class($theme, $template, $content, $request)
{
	...
	if (!as_user_maximum_permit_error('permit_view_likers_flaggers')) {
		$loadlayers[] = array(
			'directory' => AS_INCLUDE_DIR . 'plugins/',
			'include' => 'as-layer-likers-flaggers.php',
			'urltoroot' => null,
		);
	}
...
function as_get_media_html($mediaid, $height, $width, $default=null){
	$media = strlen($mediaid) ? $mediaid : (isset($default) ? $default : 'category.jpg');
	return '<img src="'.as_path_html('./as-media/' . $media) . '" height="' . $height . '" width="' . $width . '"/> ';
}
 ...
 * @return null|string
 */
function as_get_avatar_blob_html($blobId, $width, $height, $size, $class='', $padding = false)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
	...
	$html = vsprintf('<img src="%s"%s class="'.($class ? $class.' ' : '').'img-circle" style="border-radius: '.(0.75 * $size).'px" alt="user image"/>', $params);
