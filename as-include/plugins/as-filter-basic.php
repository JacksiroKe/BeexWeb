...
class as_filter_basic
{
	...
	public function filter_article(&$item, &$errors, $oldarticle)
	{
		if ($oldarticle === null) {
		...
		$this->validate_field_length($errors, $item, 'title', $qminlength, $qmaxlength);

		$this->validate_field_length($errors, $item, 'content', 0, AS_DB_MAX_CONTENT_LENGTH); // for storage
		$this->validate_field_length($errors, $item, 'text', as_opt('min_len_q_content'), null); // for display
		...
		if (isset($item['tags'])) {
			$counttags = count($item['tags']);
			$maxtags = as_opt('max_num_q_tags');
			$mintags = min(as_opt('min_num_q_tags'), $maxtags);

			if ($counttags < $mintags) {
				$errors['tags'] = as_lang_sub('item/min_tags_x', $mintags);
			} elseif ($counttags > $maxtags) {
				$errors['tags'] = as_lang_sub('item/max_tags_x', $maxtags);
			} else {
				$tagstring = as_tags_to_tagstring($item['tags']);
		...
		$this->validate_post_email($errors, $item);
	}

	public function filter_review(&$review, &$errors, $item, $oldreview)
	{
	...
	public function filter_comment(&$comment, &$errors, $item, $parent, $oldcomment)
	{
	...
	private function validate_field_length(&$errors, &$post, $key, $minlength, $maxlength, $errorKey = null)
	{
		...
		// skip the field if key not set (for example, 'title' when recategorizing items)
