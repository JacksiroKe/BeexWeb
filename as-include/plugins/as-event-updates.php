...
class as_event_updates
{
	public function process_event($event, $userid, $handle, $cookieid, $params)
	{
		...
		switch ($event) {
			case 'q_post':
				if (isset($params['parent'])) // item is following an review
			...
			case 'c_post':
				...
				foreach ($keyuserids as $keyuserid => $dummy) {
					if ($keyuserid != $userid)
						as_db_event_create_not_entity($keyuserid, $params['articleid'], $params['postid'], AS_UPDATE_FOLLOWS, $userid);
				}
				...
				as_create_event_for_q_user($params['articleid'], $params['postid'], $updatetype, $userid,
			...
			case 'q_edit':
				...
				if (isset($updatetype)) {
					as_create_event_for_q_user($params['postid'], $params['postid'], $updatetype, $userid, $params['oldarticle']['userid']);
			...
			case 'q_reopen':
			case 'q_close':
				as_create_event_for_q_user($params['postid'], $params['postid'], AS_UPDATE_CLOSED, $userid, $params['oldarticle']['userid']);
				break;


			case 'q_hide':
				if (isset($params['oldarticle']['userid']))
					as_db_event_create_not_entity($params['oldarticle']['userid'], $params['postid'], $params['postid'], AS_UPDATE_VISIBLE, $userid);
				break;


			case 'q_reshow':
				as_create_event_for_q_user($params['postid'], $params['postid'], AS_UPDATE_VISIBLE, $userid, $params['oldarticle']['userid']);
				break;


			case 'q_move':
				as_create_event_for_q_user($params['postid'], $params['postid'], AS_UPDATE_CATEGORY, $userid, $params['oldarticle']['userid']);
			...
			case 'c_edit':
				if ($params['contentchanged'])
					as_create_event_for_q_user($params['articleid'], $params['postid'], AS_UPDATE_CONTENT, $userid, $params['oldcomment']['userid']);
				break;


			case 'a_to_c':
				if ($params['contentchanged'])
					as_create_event_for_q_user($params['articleid'], $params['postid'], AS_UPDATE_CONTENT, $userid, $params['oldreview']['userid']);
				else
					as_create_event_for_q_user($params['articleid'], $params['postid'], AS_UPDATE_TYPE, $userid, $params['oldreview']['userid']);
				break;


			case 'c_hide':
				if (isset($params['oldcomment']['userid']))
					as_db_event_create_not_entity($params['oldcomment']['userid'], $params['articleid'], $params['postid'], AS_UPDATE_VISIBLE, $userid);
				break;


			case 'c_reshow':
				as_create_event_for_q_user($params['articleid'], $params['postid'], AS_UPDATE_VISIBLE, $userid, $params['oldcomment']['userid']);
				break;
		}
	}
}
