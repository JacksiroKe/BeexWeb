...
{
	public function index_post($postid, $type, $articleid, $parentid, $title, $content, $format, $text, $tagstring, $categoryid)
	{
		...
		as_db_contentwords_add_post_wordidcounts($postid, $type, $articleid, $contentwordidcounts);
	...
	public function process_search($query, $start, $count, $userid, $absoluteurls, $fullcontent)
	{
		...
		foreach ($items as $item) {
			as_search_set_max_match($item, $type, $postid); // to link straight to best part
