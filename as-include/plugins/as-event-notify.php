	...
	public function process_event($event, $userid, $handle, $cookieid, $params)
	{
		...
		switch ($event) {
			...
			case 'a_post':
				...
				if (isset($item['notify']) && !as_post_is_by_user($item, $userid, $cookieid))
					as_send_notification($item['userid'], $item['notify'], @$item['handle'], as_lang('emails/q_reviewed_subject'), as_lang('emails/q_reviewed_body'), array(
						...
						'^url' => as_q_path($item['postid'], $item['title'], true, 'R', $params['postid']),
					));
				break;
			...
			case 'c_post':
				...
				$sendurl = as_q_path($item['postid'], $item['title'], true, 'C', $params['postid']);
					...
					as_send_notification(null, as_opt('feedback_email'), null,
						...
						array(
							'^p_handle' => isset($handle) ? $handle : (strlen($params['name']) ? $params['name'] :
								(strlen(@$oldarticle['name']) ? $oldarticle['name'] : as_lang('main/anonymous'))),
						...
						array(
							'^p_handle' => isset($handle) ? $handle : (strlen($params['name']) ? $params['name'] :
								(strlen(@$oldcomment['name']) ? $oldcomment['name'] : // could also be after review converted to comment
									(strlen(@$oldreview['name']) ? $oldreview['name'] : as_lang('main/anonymous')))),
							'^p_context' => $params['text'], // don't censor for admin
							'^url' => as_q_path($params['articleid'], $params['item']['title'], true, 'C', $params['postid']),
							'^a_url' => as_path_absolute('admin/moderate'),
						)
					);
				}
			...
			case 'c_flag':
				...
				if ($notifycount >= 0 && ($notifycount % as_opt('flagging_notify_every')) == 0) {
					as_send_notification(null, as_opt('feedback_email'), null, as_lang('emails/flagged_subject'), as_lang('emails/flagged_body'), array(
						...
						'^url' => as_q_path($params['articleid'], $params['item']['title'], true, $oldpost['basetype'], $oldpost['postid']),
