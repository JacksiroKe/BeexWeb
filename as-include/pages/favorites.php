...
list($numQs, $items, $numUsers, $users, $numTags, $tags, $categories) = as_db_select_with_pending(
...
$usershtml = as_userids_handles_html(AS_FINAL_EXTERNAL_USERS ? $items : array_merge($items, $users));
...
$as_content['p_list'] = as_favorite_p_list_view($items, $usershtml);
$as_content['p_list']['title'] = count($items) ? as_lang_html('main/nav_qs') : as_lang_html('misc/no_favorite_qs');
if ($numQs['count'] > count($items)) {
	$url = as_path_html('favorites/items', array('start' => $pagesize_qs));
	$as_content['p_list']['footer'] = '<p class="as-link-next"><a href="' . $url . '">' . as_lang_html('misc/more_favorite_qs') . '</a></p>';
}
