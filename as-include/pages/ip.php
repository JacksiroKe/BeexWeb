...
// Find recently (hidden, queued or not) items, reviews, comments and edits for this IP
...
list($qs, $qs_queued, $qs_hidden, $a_qs, $a_queued_qs, $a_hidden_qs, $c_qs, $c_queued_qs, $c_hidden_qs, $edit_qs) =
	as_db_select_with_pending(
		as_db_question_selectspec($userid, 'created', 0, null, $ip, false),
		as_db_question_selectspec($userid, 'created', 0, null, $ip, 'P_QUEUED'),
		as_db_question_selectspec($userid, 'created', 0, null, $ip, 'P_HIDDEN', true),
...
// Combine sets of items and get information for users

$items = as_any_sort_by_date(array_merge($qs, $qs_queued, $qs_hidden, $a_qs, $a_queued_qs, $a_hidden_qs, $c_qs, $c_queued_qs, $c_hidden_qs, $edit_qs));
...
if ($blockable) {
	...
	if (count($matchclauses)) {
		...
		if (count($items) && !as_user_maximum_permit_error('permit_hide_show'))
...
if (count($items)) {
	$as_content['p_list']['title'] = as_lang_html_sub('misc/recent_activity_from_x', as_html($ip));
	...
	foreach ($items as $item) {
		...
		$hasother = isset($item['opostid']);

		if ($item[$hasother ? 'ohidden' : 'hidden'] && !isset($item[$hasother ? 'oupdatetype' : 'updatetype'])) {
			$htmlfields['what_2'] = as_lang_html('main/hidden');

			if (@$htmloptions['whenview']) {
				$updated = @$item[$hasother ? 'oupdated' : 'updated'];
				if (isset($updated))
	...
	$as_content['p_list']['title'] = as_lang_html_sub('misc/no_activity_from_x', as_html($ip));
