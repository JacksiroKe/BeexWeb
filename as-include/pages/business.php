...
if (is_numeric($request)) {
	...
	if (!isset($editdepartid)) $editdepartid = as_get('edit');
	if (!isset($editdepartid)) $editdepartid = as_get('addsub');
	...
	if (as_clicked('dosaveoptions')) {
		if (!as_check_form_security_code('business-departments', as_post_text('code')))
			$securityexpired = true;

		else {
			as_set_option('allow_no_department', (int)as_post_text('option_allow_no_department'));
			as_set_option('allow_no_sub_department', (int)as_post_text('option_allow_no_sub_department'));
			$savedoptions = true;
		}
	}
	...
	if (as_clicked('docancel')) {
		if ($setmissing || $setparent)
			as_redirect(as_request(), array('edit' => $editdepartment['departid']));
		elseif (isset($editdepartment['departid']))
		...
		if (!as_check_form_security_code('business-departments', as_post_text('code')))
			...
			as_db_department_reassign($editdepartment['departid'], $inreassign);
			as_redirect(as_request(), array('recalc' => 1, 'edit' => $editdepartment['departid']));
		}
		...
		elseif (as_post_text('dodelete')) {
			if (!$hassubdepartment) {
				$inreassign = as_get_department_field_value('reassign');
				as_db_department_reassign($editdepartment['departid'], $inreassign);
				as_db_department_delete($editdepartment['departid']);
			...
			elseif (as_strlen($inname) > AS_DB_MAX_CAT_PAGE_TITLE_LENGTH)
				...
				foreach ($indepartments as $department) {
					if (!strcmp($department['parentid'], $inparentid) &&
						strcmp($department['departid'], @$editdepartment['departid']) &&
			...
			for ($attempt = 0; $attempt < 100; $attempt++) {
				...
				$matchdepartid = as_db_department_slug_to_id($inparentid, $inslug); // query against DB since MySQL ignores accents, etc...
				...
				if (isset($editdepartment['departid']) || !isset($errors['slug'])) // don't try other options if editing existing department
					break;
			}
			...
			if (empty($errors)) {
				...
				if (isset($editdepartment['departid'])) { // changing existing department
					...
					if (isset($inposition))
						as_db_department_set_position($departid, $inposition);
	...
	if ($setmissing) {
		...
		if ($setparent) {
			...
			as_set_up_department_field($as_content, $formcontent['fields']['parent'], 'parent',
				...
				true, true, AS_CATEGORY_DEPTH - 1 - $childdepth, @$editdepartment['departid']);
			...
			if ($hassubdepartment) {
				...
				$formcontent['fields']['delete'] = array(
					...
					'label' =>
						'<span id="reassign_shown">' . as_lang_html('main/delete_department_reassign') . '</span>' .
						'<span id="reassign_hidden" style="display:none;">' . as_lang_html('main/delete_department') . '</span>',
					'value' => 0,
					'type' => 'checkbox',
				);
			...
			if ($hassubdepartment && !as_opt('allow_no_sub_department')) {
				$nosubcount = as_db_count_departid_qs($editdepartment['departid']);
		...
		if (!$setparent) {
			...
			foreach ($departments as $key => $department) {
				if (!strcmp($department['parentid'], @$editdepartment['parentid'])) {
					if (isset($previous))
						$positionhtml = as_lang_html_sub('main/after_x', as_html($passedself ? $department['title'] : $previous['title']));
					else
						$positionhtml = as_lang_html('main/first');
			...
			if (isset($editdepartment['position']))
				...
				$positionvalue = isset($previous) ? as_lang_html_sub('main/after_x', as_html($previous['title'])) : as_lang_html('main/first');
			...
			$formcontent['fields']['position'] = array(
				'id' => 'position_display',
				'tags' => 'name="position"',
				'label' => as_lang_html('main/position'),
				'type' => 'select',
		...
		$formcontent = array(
			...
			/*'fields' => array(
				'intro' => array(
					'label' => as_lang_html('main/departments_introduction'),
					'type' => 'static',
				),
			),*/
	...
	if (as_get('recalc')) {
		$formcontent['ok'] = '<span id="recalc_ok">' . as_lang_html('main/recalc_departments') . '</span>';
