...
if ((as_clicked('doaddpage') || as_clicked('doaddlink') || as_get('doaddlink') || as_clicked('dosavepage')) && !isset($editpage)) {
	...
	$igenderternal = as_clicked('doaddlink') || as_get('doaddlink') || as_post_text('external');

} elseif (isset($editpage))
	$igenderternal = $editpage['flags'] & AS_PAGE_FLAGS_EXTERNAL;
...
$navoptions = array(
	...
	'nav_articles' => 'main/nav_qs',
	'nav_orders' => 'main/nav_orders',
...
elseif (as_clicked('dosaveoptions') || as_clicked('doaddpage') || as_clicked('doaddlink')) {
	...
	if (!as_check_form_security_code('admin/pages', as_post_text('code')))
		...
		if (as_post_text('dodelete')) {
			...
			if (as_clicked('dosaveview') && empty($errors) && !$igenderternal)
				as_redirect($inslug);
		}
...
if (isset($editpage)) {
	...
	$as_content['form'] = array(
		...
		'fields' => array(
			'name' => array(
				...
				'label' => as_lang_html($igenderternal ? 'admin/link_name' : 'admin/page_name'),
			...
			'delete' => array(
				...
				'label' => as_lang_html($igenderternal ? 'admin/delete_link' : 'admin/delete_page'),
				'value' => 0,
				'type' => 'checkbox',
			),
		...
		'buttons' => array(
			'save' => array(
				'label' => as_lang_html(isset($editpage['pageid']) ? 'main/save_button' : ($igenderternal ? 'admin/add_link_button' : 'admin/add_page_button')),
			),
	...
	if (isset($editpage['pageid'])) {
		as_set_display_rules($as_content, array(
			...
			($igenderternal ? 'url_display' : 'slug_display') => '!dodelete',
			($igenderternal ? 'newwindow_display' : 'heading_display') => '!dodelete',
	...
	if ($igenderternal || !isset($editpage['pageid']))
