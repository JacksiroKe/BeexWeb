...
$optiontype = array(
	...
	'page_size_write_check_qs' => 'number',
	'page_size_write_tags' => 'number',
	...
	'allow_close_articles' => 'checkbox',
	'allow_close_own_articles' => 'checkbox',
...
$optionmaximum = array(
	...
	'page_size_write_check_qs' => AS_DB_RETRIEVE_QS_AS,
	'page_size_write_tags' => AS_DB_RETRIEVE_QS_AS,
...
$optionminimum = array(
	...
	'page_size_write_check_qs' => 3,
	'page_size_write_tags' => 3,
	'page_size_home' => 1,
	'page_size_hot_qs' => 1,
	'page_size_pms' => 1,
	'page_size_q_as' => 1,
...
switch ($adminsection) {
	...
	case 'users':
		...
		array_push($showoptions, 'avatar_users_size', 'avatar_q_page_q_size', 'avatar_q_page_a_size', 'avatar_q_page_c_size',
			'avatar_p_list_size', 'avatar_message_list_size');
		...
		if (!AS_FINAL_EXTERNAL_USERS) {
			$checkboxtodisplay = array_merge($checkboxtodisplay, array(
				...
				'avatar_p_list_size' => 'option_avatar_allow_gravatar || option_avatar_allow_upload',
	...
	case 'viewing':
		...
		$showoptions = array(
			...
			'voting_on_qs', 'voting_on_q_page_only', 'voting_on_as', 'voting_on_cs', 'likes_separated', '',
		...
		foreach ($widgets as $widget) {
			if ($widget['title'] == 'Related Articles') {
		...
		$checkboxtodisplay = array(
			...
			'likes_separated' => 'option_voting_on_qs || option_voting_on_as',
	...
	case 'lists':
		...
		array_push($showoptions, '', 'admin/hotness_factors', 'hot_weight_q_age', 'hot_weight_a_age', 'hot_weight_reviews', 'hot_weight_likes');
	...
	case 'posting':
		...
		$showoptions = array('do_close_on_select', 'allow_close_articles', 'allow_close_own_articles', 'allow_self_review', 'allow_multi_reviews', 'follow_on_as', 'comment_on_qs', 'comment_on_as', 'allow_anonymous_naming', '');

		if (count(as_list_modules('editor')) > 1)
			array_push($showoptions, 'editor_for_qs', 'editor_for_as', 'editor_for_cs', '');

		array_push($showoptions, 'show_custom_write', 'custom_write', 'extra_field_active', 'extra_field_prompt', 'extra_field_display', 'extra_field_label', 'show_custom_review', 'custom_review', 'show_custom_comment', 'custom_comment', '');
		...
		array_push($showoptions, '', 'block_bad_words', '', 'do_write_check_qs', 'match_write_check_qs', 'page_size_write_check_qs', '');

		if (as_using_tags())
			array_push($showoptions, 'do_example_tags', 'match_example_tags', 'do_complete_tags', 'page_size_write_tags');

		$formstyle = 'wide';

		$checkboxtodisplay = array(
			'allow_close_own_articles' => 'option_allow_close_articles',
			'editor_for_cs' => 'option_comment_on_qs || option_comment_on_as',
			'custom_write' => 'option_show_custom_write',
			...
			'match_write_check_qs' => 'option_do_write_check_qs',
			'page_size_write_check_qs' => 'option_do_write_check_qs',
			'match_example_tags' => 'option_do_example_tags',
			'page_size_write_tags' => 'option_do_example_tags || option_do_complete_tags',
		);
		break;
	...
	case 'feeds':
		...
		$showoptions = array('feed_for_articles', 'feed_for_as', 'feed_for_activity');
		...
		$checkboxtodisplay = array(
			'feed_per_category' => 'option_feed_for_as || option_feed_for_articles || option_feed_for_unreviewed || option_feed_for_activity',
		);
		break;
	...
	case 'spam':
		...
		if (as_opt('voting_on_qs') || as_opt('voting_on_as') || as_opt('voting_on_cs'))
			array_push($showoptions, 'max_rate_ip_likes', 'max_rate_user_likes');
...
foreach ($showoptions as $optionname) {
	if (empty($optionname)) {
		...
		switch ($optionname) { // special treatment for certain options
				...
				foreach ($rawoptions as $rawoption) {
					$neatoptions[$rawoption] =
						...
						(($rawoption == AS_URL_FORMAT_NEAT) ? strtr(as_lang_html('admin/neat_urls_note'), array(
							'^1' => '<a href="http://www.appsmata.org/htaccess.php" target="_blank">',
							'^2' => '</a>',
						)) : '') .
						'</small>';
				}
			...
			case 'sort_reviews_by':
				as_optionfield_make_select($optionfield, array(
					...
					'likes' => as_lang_html('options/sort_likes'),
				), $value, 'created');
				break;
			...
			case 'permit_view_likers_flaggers':
			case 'permit_post_wall':
				...
				elseif ($optionname == 'permit_view_likers_flaggers' || $optionname == 'permit_edit_silent')
				...
				elseif ($optionname == 'permit_like_q' || $optionname == 'permit_like_a' || $optionname == 'permit_like_c' || $optionname == 'permit_post_wall')
				...
				elseif ($optionname == 'permit_view_likers_flaggers')
					$narrowest = AS_PERMIT_SUPERS;
				else
			...
			case 'permit_like_q_points':
			case 'permit_like_a_points':
			case 'permit_like_c_points':
			case 'permit_like_down_points':
			...
			case 'feed_per_category':
				...
				if (as_opt('feed_for_as'))
					$feedrequest = 'as';
				elseif (as_opt('feed_for_articles'))
					$feedrequest = 'items';
				else
					$feedrequest = 'activity';

				$feedrequest .= '/' . $categoryslug;
				$feedigenderample = true;
				break;
		...
		if (isset($feedrequest) && $value) {
			$optionfield['note'] = '<a href="' . as_path_html(as_feed_request($feedrequest)) . '">' . as_lang_html($feedigenderample ? 'admin/feed_link_example' : 'admin/feed_link') . '</a>';
		}
