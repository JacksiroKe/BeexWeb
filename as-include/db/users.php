...
{
	as_db_query_sub(
			'INSERT INTO ^suppliers (userid, title, content, field1, field2) ' .
			'VALUES (#, $, $, $, $)',
			$userid, $title, $content, $field1, $field2
		);
 ...
 * @return mixed
 */
function as_db_user_create($type, $firstname, $lastname, $gender, $country, $mobile, $email, $password, $handle, $level, $ip)
{
	...
	if (AS_PASSWORD_HASH) {
		as_db_query_sub(
			'INSERT INTO ^users (created, createip, type, profiles, firstname, lastname, gender, country, mobile, email, passhash, level, handle, signedin, signinip) ' .
			'VALUES (NOW(), UNHEX($), $, $, $, $, #, $, $, $, $, #, $, NOW(), UNHEX($))',
			$ipHex, $type, $type, $firstname, $lastname, $gender, $country, $mobile, $email, isset($password) ? password_hash($password, PASSWORD_BCRYPT) : null, (int)$level, $handle, $ipHex
		);
	} else {
		$salt = isset($password) ? as_random_alphanum(16) : null;

		as_db_query_sub(
			'INSERT INTO ^users (created, createip, type, profiles, firstname, lastname, gender, country, mobile, email, passsalt, passcheck, level, handle, signedin, signinip) ' .
			'VALUES (NOW(), UNHEX($), $, $, $, $, #, $, $, $, $, UNHEX($), #, $, NOW(), UNHEX($))',
			$ipHex, $type, $type, $firstname, $lastname, $gender, $country, $mobile, $email, $salt, isset($password) ? as_db_calc_passcheck($password, $salt) : null, (int)$level, $handle, $ipHex
		);
	}
...
function as_db_user_delete($userid)
{
	...
	as_db_query_sub('DELETE FROM ^userlikes WHERE userid=$', $userid);
...
function as_db_name_find_by_handle($handle)
{
	...
	return isset($username[0]) ? $username[0] : '';
}
