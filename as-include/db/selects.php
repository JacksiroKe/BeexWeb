 ...
 * @return array
 */
function as_db_posts_basic_selectspec($likeuserid = null, $full = false, $user = true)
{
	...
	if (isset($likeuserid)) {
		...
		$selectspec['columns']['userlike'] = '^userlikes.lyke';
		$selectspec['columns']['userflag'] = '^userlikes.flag';
		$selectspec['columns']['userfavoriteq'] = '^userfavorites.entityid<=>^posts.postid';
		$selectspec['source'] .= ' LEFT JOIN ^userlikes ON ^posts.postid=^userlikes.postid AND ^userlikes.userid=$';
		...
		array_push($selectspec['arguments'], $likeuserid, $likeuserid, AS_ENTITY_ARTICLE);
	}
 ...
 * comment) which is related to the main post (item) retrieved. Pass the name of table which will contain the other
 ...
 * post (review or comment) which is related to the main post (item) retrieved. Pass the name of table which will
...
function as_db_os_selectspec($likeuserid, $count = null, $start = 0)
{
	...
	$selectspec = as_db_orders_basic_selectspec($likeuserid);

	$selectspec['source'] .= " ORDER BY ^orders.created DESC LIMIT #,#";
...
/**
 * Return the selectspec to retrieve recent items (of type $specialtype if provided, or 'P' by default) which,
 * depending on $by, either (a) have no reviews, (b) have on selected reviews, or (c) have no positiveliked reviews. The
 * items are restricted to the category for $categoryslugs (if not null), and will have the corresponding like made
 * by $likeuserid (if not null) and will include $full content or not. Return $count (if null, a default is used)
 * items starting from offset $start.
 * @param $likeuserid
 * @param $by
 * @param $start
 ...
 * @return array
 */
function as_db_unreviewed_qs_selectspec($likeuserid, $by, $start, $categoryslugs = null, $specialtype = false, $full = false, $count = null)
{
	if ($specialtype == 'P' || $specialtype == 'P_QUEUED') {
		$type = $specialtype;
	} else {
...
/**
 * Return the selectspec to retrieve the antecedent items for recent reviews (of type $specialtype if provided, or
 * 'R' by default), restricted to $createip (if not null) and the category for $categoryslugs (if not null), with the
 * corresponding like on those items made by $likeuserid (if not null). Return $count (if null, a default is used)
 * items starting from offset $start. The selectspec will also retrieve some information about the reviews
 ...
 * @return array
 */
function as_db_recent_a_qs_selectspec($likeuserid, $start, $categoryslugs = null, $createip = null, $specialtype = false, $fullreviews = false, $count = null)
{
	...
	$selectspec = as_db_posts_basic_selectspec($likeuserid);
	...
	$selectspec['source'] .=
		...
		($specialtype ? '' : " WHERE ^posts.type='P'");
...
/**
 * Return the selectspec to retrieve the antecedent items for recent comments (of type $specialtype if provided, or
 * 'C' by default), restricted to $createip (if not null) and the category for $categoryslugs (if not null), with the
 * corresponding like on those items made by $likeuserid (if not null). Return $count (if null, a default is used)
 * items starting from offset $start. The selectspec will also retrieve some information about the comments
 ...
 * @return array
 */
function as_db_recent_c_qs_selectspec($likeuserid, $start, $categoryslugs = null, $createip = null, $specialtype = false, $fullcomments = false, $count = null)
{
	...
	$selectspec['source'] .=
		...
		($specialtype ? '' : " WHERE ^posts.type='P' AND ((parentposts.type='P') OR (parentposts.type='R'))");
...
/**
 * Return the selectspec to retrieve the antecedent items for recently edited posts, restricted to edits by $lastip
 * (if not null), the category for $categoryslugs (if not null) and only visible posts (if $onlyvisible), with the
 * corresponding like on those items made by $likeuserid (if not null). Return $count (if null, a default is used)
 * items starting from offset $start. The selectspec will also retrieve some information about the edited posts
 ...
 * @return array
 */
function as_db_recent_edit_qs_selectspec($likeuserid, $start, $categoryslugs = null, $lastip = null, $onlyvisible = true, $fulledited = false, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
	...
	$selectspec['source'] .=
		" JOIN ^posts AS parentposts ON" .
		" ^posts.postid=IF(LEFT(parentposts.type, 1)='P', parentposts.postid, parentposts.parentid)" .
		" JOIN ^posts AS editposts ON parentposts.postid=IF(LEFT(editposts.type, 1)='P', editposts.postid, editposts.parentid)" .
		...
		($onlyvisible ? "type IN ('P', 'R', 'C')" : "1") .
		" ORDER BY ^posts.updated DESC LIMIT #,#) y ON editposts.postid=y.postid" .
		($onlyvisible ? " WHERE parentposts.type IN ('P', 'R', 'C') AND ^posts.type IN ('P', 'R', 'C')" : "");
...
/**
 * Return the selectspec to retrieve the antecedent items for the most flagged posts, with the corresponding like
 * on those items made by $likeuserid (if not null). Return $count (if null, a default is used) items starting
 ...
 * @return array
 */
function as_db_flagged_post_qs_selectspec($likeuserid, $start, $fullflagged = false, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
	...
	$selectspec['source'] .=
		" JOIN ^posts AS parentposts ON" .
		" ^posts.postid=IF(LEFT(parentposts.type, 1)='P', parentposts.postid, parentposts.parentid)" .
		" JOIN ^posts AS flagposts ON parentposts.postid=IF(LEFT(flagposts.type, 1)='P', flagposts.postid, flagposts.parentid)" .
		...
		" JOIN (SELECT postid FROM ^posts WHERE flagcount>0 AND type IN ('P', 'R', 'C') ORDER BY ^posts.flagcount DESC, ^posts.created DESC LIMIT #,#) y ON flagposts.postid=y.postid";
...
/**
 * Return the selectspec to retrieve the posts in $postids, with the corresponding like on those posts made by
 * $likeuserid (if not null). Returns full information if $full is true.
 * @param $likeuserid
 * @param $postids
 * @param bool $full
 * @return array
 */
function as_db_posts_selectspec($likeuserid, $postids, $full = false)
{
	$selectspec = as_db_posts_basic_selectspec($likeuserid, $full);
...
{
	return array(
		'columns' => array('businessid', 'type', 'title', 'contact', 'location', 'username', 'content', 'icon', 'images', 'tags', 'userid', 'created'),
...
{
	$selectspec['columns'] = array('businessid', '^businesses.type', 'title', 'contact', 'location', 'username', 'content', 'icon', 'images', 'tags', '^businesses.userid', 'created' => 'UNIX_TIMESTAMP(^businesses.created)', 'updated' => 'UNIX_TIMESTAMP(^businesses.updated)');
...
{
	return array(
		'columns' => array('departid', 'businessid', 'parentid', 'title', 'icon', 'content', 'userid', 'managers', 'users', 'extra', 'created'),
 ...
 * @return array
 */
function as_db_posts_to_qs_selectspec($likeuserid, $postids, $full = false)
{
	$selectspec = as_db_posts_basic_selectspec($likeuserid, $full);
	...
	$selectspec['source'] .=
		" JOIN ^posts AS parentposts ON" .
		" ^posts.postid=IF(LEFT(parentposts.type, 1)='P', parentposts.postid, parentposts.parentid)" .
		" JOIN ^posts AS childposts ON parentposts.postid=IF(LEFT(childposts.type, 1)='P', childposts.postid, childposts.parentid)" .
...
/**
 * Return the selectspec to retrieve the full information for $postid, with the corresponding like made by $likeuserid (if not null)
 * @param $likeuserid
 * @param $postid
 * @return array
 */
function as_db_full_post_selectspec($likeuserid, $postid)
{
	$selectspec = as_db_posts_basic_selectspec($likeuserid, true);
 ...
 * corresponding like made by $likeuserid (if not null)
 * @param $likeuserid
 * @param $parentid
 * @return array
 */
function as_db_full_child_posts_selectspec($likeuserid, $parentid)
{
	$selectspec = as_db_posts_basic_selectspec($likeuserid, true);
 ...
 * has $articleid as its parent, with the corresponding like made by $likeuserid (if not null)
 * @param $likeuserid
 * @param $articleid
 * @return array
 */
function as_db_full_a_child_posts_selectspec($likeuserid, $articleid)
{
	$selectspec = as_db_posts_basic_selectspec($likeuserid, true);
...
/**
 * Return the selectspec to retrieve the item for the parent of $postid (where $postid is of a follow-on item or comment),
 * i.e. the parent of $articleid's parent if $articleid's parent is an review, otherwise $articleid's parent itself.
 * @param $postid
 * @return array
 */
function as_db_post_parent_q_selectspec($postid)
{
	...
	$selectspec['source'] .= " WHERE ^posts.postid=(SELECT IF(LEFT(parent.type, 1)='R', parent.parentid, parent.postid) FROM ^posts AS child LEFT JOIN ^posts AS parent ON parent.postid=child.parentid WHERE child.postid=# AND parent.type IN('P','R'))";
...
/**
 * Return the selectspec to retrieve the post (either duplicate item or explanatory note) which has closed $articleid, if any
 * @param $articleid
 * @return array
 */
...
/**
 * Return the selectspec to retrieve the posts that have been closed as a duplicate of this item, if any
 * @param $articleid int The canonical item.
 * @return array
 */
...
/**
 * Return the selectspec to retrieve the most closely related items to $articleid, with the corresponding like
 * made by $likeuserid (if not null). Return $count (if null, a default is used) items. This works by looking for
 * other items which have title words, tag words or an (exact) category in common.
 * @param $likeuserid
 * @param $articleid
 * @param $count
 * @return array
 */
function as_db_related_qs_selectspec($likeuserid, $articleid, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
...
/**
 * Return the selectspec to retrieve the top item matches for a search, with the corresponding like made by
 * $likeuserid (if not null) and including $full content or not. Return $count (if null, a default is used) items
 * starting from offset $start. The search is performed for any of $titlewords in the title, $contentwords in the
 * content (of the item or an review or comment for whom that is the antecedent item), $tagwords in tags, for
 * item author usernames which match a word in $handlewords or which match $handle as a whole. The results also
 * include a 'score' column based on the matching strength and post hotness, and a 'matchparts' column that tells us
 * where the score came from (since a item could get weight from a match in the item itself, and/or weight from
 ...
 * @return array
 */
function as_db_search_posts_selectspec($likeuserid, $titlewords, $contentwords, $tagwords, $handlewords, $handle, $start, $full = false, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;

	// add LOG(postid)/1000000 here to ensure ordering is deterministic even if several posts have same score
	// The score also gives a bonus for hot items, where the bonus scales linearly with hotness. The hottest
	// item gets a bonus equivalent to a matching unique tag, and the least hot item gets zero bonus.

	$selectspec = as_db_posts_basic_selectspec($likeuserid, $full);

	$selectspec['columns'][] = 'score';
	$selectspec['columns'][] = 'matchparts';
	$selectspec['source'] .= " JOIN (SELECT articleid, SUM(score)+2*(LOG(#)*(MAX(^posts.hotness)-(SELECT MIN(hotness) FROM ^posts WHERE type='P'))/((SELECT MAX(hotness) FROM ^posts WHERE type='P')-(SELECT MIN(hotness) FROM ^posts WHERE type='P')))+LOG(articleid)/1000000 AS score, GROUP_CONCAT(CONCAT_WS(':', matchposttype, matchpostid, ROUND(score,3))) AS matchparts FROM (";
	$selectspec['sortdesc'] = 'score';
	...
	if (!empty($titlewords)) {
		...
		$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
			"(SELECT postid AS articleid, LOG(#/titlecount) AS score, 'P' AS matchposttype, postid AS matchpostid FROM ^titlewords JOIN ^words ON ^titlewords.wordid=^words.wordid WHERE word IN ($) AND titlecount<#)";
	...
	if (!empty($contentwords)) {
		...
		// On top of that, review matches are worth half a item match, and comment/note matches half again.

		$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
			"(SELECT articleid, (1-1/(1+count))*LOG(#/contentcount)*(CASE ^contentwords.type WHEN 'P' THEN 1.0 WHEN 'R' THEN 0.5 ELSE 0.25 END) AS score, ^contentwords.type AS matchposttype, ^contentwords.postid AS matchpostid FROM ^contentwords JOIN ^words ON ^contentwords.wordid=^words.wordid WHERE word IN ($) AND contentcount<#)";
	...
	if (!empty($tagwords)) {
		...
		$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
			"(SELECT postid AS articleid, 2*LOG(#/tagwordcount) AS score, 'P' AS matchposttype, postid AS matchpostid FROM ^tagwords JOIN ^words ON ^tagwords.wordid=^words.wordid WHERE word IN ($) AND tagwordcount<#)";
	...
	if (!empty($handlewords)) {
		if (AS_FINAL_EXTERNAL_USERS) {
			...
			if (count($userids)) {
				$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
					"(SELECT postid AS articleid, LOG(#/qposts) AS score, 'P' AS matchposttype, postid AS matchpostid FROM ^posts JOIN ^userpoints ON ^posts.userid=^userpoints.userid WHERE ^posts.userid IN ($) AND type='P')";
			...
			$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
				"(SELECT postid AS articleid, LOG(#/qposts) AS score, 'P' AS matchposttype, postid AS matchpostid FROM ^posts JOIN ^users ON ^posts.userid=^users.userid JOIN ^userpoints ON ^userpoints.userid=^users.userid WHERE handle IN ($) AND type='P')";
	...
	if (strlen($handle)) { // to allow searching for multi-word usernames (only works if search query contains full username and nothing else)
		if (AS_FINAL_EXTERNAL_USERS) {
			...
			if (count($userids)) {
				$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
					"(SELECT postid AS articleid, LOG(#/qposts) AS score, 'P' AS matchposttype, postid AS matchpostid FROM ^posts JOIN ^userpoints ON ^posts.userid=^userpoints.userid WHERE ^posts.userid=$ AND type='P')";
			...
			$selectspec['source'] .= ($selectparts++ ? " UNION ALL " : "") .
				"(SELECT postid AS articleid, LOG(#/qposts) AS score, 'P' AS matchposttype, postid AS matchpostid FROM ^posts JOIN ^users ON ^posts.userid=^users.userid JOIN ^userpoints ON ^userpoints.userid=^users.userid WHERE handle=$ AND type='P')";
...
/**
 * Processes the matchparts column in $item which was returned from a search performed via as_db_search_posts_selectspec()
 * Returns the id of the strongest matching review or comment, or null if the item itself was the strongest match
 * @param $item
 * @param $type
 * @param $postid
 * @return null
 */
function as_search_set_max_match($item, &$type, &$postid)
{
	$type = 'P';
	$postid = $item['postid'];
	$bestscore = null;

	$matchparts = explode(',', $item['matchparts']);
...
function as_db_full_category_selectspec($slugsorid, $isid)
{
	...
	return array(
		'columns' => array('categoryid', 'parentid', 'title', 'tags', 'pcount', 'content', 'backpath'),
...
function as_db_list_orders_selectspec() 
{
	$selectspec['columns'] = array('orderid', 'itemid', '^orders.userid', '^orders.quantity', 'address', '^posts.saleprice', 'icon' => '^posts.icon', 
			'created' => 'UNIX_TIMESTAMP(^orders.created)', 'updated' => 'UNIX_TIMESTAMP(^orders.updated)',
			'categoryname' => '(SELECT title FROM ^categories WHERE ^categories.categoryid=^posts.catidpath1)', 
			'caticon' => '(SELECT icon FROM ^categories WHERE ^categories.categoryid=^posts.catidpath1)',
			'orderedby' => "CONCAT_WS(' ', ^users.firstname, ^users.lastname)",
			'^posts.volume', '^posts.weight', '^posts.buyprice', '^posts.saleprice',
		);
			
	$selectspec['source'] = '^orders LEFT JOIN ^posts ON ^posts.postid=^orders.itemid';
	$selectspec['source'] .= ' LEFT JOIN ^users ON ^users.userid=^orders.userid';
	$selectspec['source'] .= ' LEFT JOIN ^categories ON ^categories.categoryid=^posts.categoryid';
	$selectspec['sortdesc'] = 'created';
	$selectspec['sortdesc_2'] = 'orderid';
...
function as_db_orders_basic_selectspec($likeuserid = null, $full = false, $user = true)
{
	...
	$selectspec = array(
		'columns' => array(
			'orderid', 'itemid', '^orders.userid', '^orders.quantity', 'address', '^posts.saleprice', 'isorder' => '^orders.orderid', 
			'orderedby' => "CONCAT_WS(' ', ^users.firstname, ^users.lastname)",
			'^posts.postid', '^posts.categoryid', '^posts.type', 'basetype' => 'LEFT(^posts.type, 1)',
			'hidden' => "INSTR(^posts.type, '_HIDDEN')>0", 'queued' => "INSTR(^posts.type, '_QUEUED')>0",
			'^posts.rcount', '^posts.selchildid', '^posts.closedbyid', '^posts.positivelikes', '^posts.negativelikes', '^posts.netlikes', '^posts.views', '^posts.hotness', '^posts.flagcount', '^posts.catidpath1',
			'icon' => '^posts.icon', 'title' => '^categories.title', '^posts.volume', '^posts.weight', '^posts.buyprice', '^posts.saleprice', '^posts.state', '^posts.color', '^posts.texture', '^posts.stockin', '^posts.stockout', '^posts.manufacturer', '^posts.images', 'created' => 'UNIX_TIMESTAMP(^orders.created)', '^posts.name', 'summary' => '^posts.content', 'categorybackpath' => "^categories.backpath", 
			'categoryname' => '(SELECT title FROM ^categories WHERE ^categories.categoryid=^posts.catidpath1)', 
			'caticon' => '(SELECT icon FROM ^categories WHERE ^categories.categoryid=^posts.catidpath1)', 
			'categoryids' => "CONCAT_WS(',', ^posts.catidpath1, ^posts.catidpath2, ^posts.catidpath3, ^posts.categoryid)",
		),
		...
		'arraykey' => 'postid',
		'source' => '^posts LEFT JOIN ^categories ON ^categories.categoryid=^posts.categoryid LEFT JOIN ^orders ON ^orders.itemid=^posts.postid',
		'arguments' => array(),
	);
...
function as_db_user_recent_orders_selectspec($likeuserid, $identifier, $count = null, $start = 0)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;

	$selectspec = as_db_orders_basic_selectspec($likeuserid);

	$selectspec['source'] .= " WHERE ^orders.userid=" . (AS_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") . " ORDER BY ^orders.created DESC LIMIT #,#";
...
function as_db_category_selectspec()
{
	$parentselects = array( // requires AS_CATEGORY_DEPTH=4
		'SELECT NULL AS parentkey', // top level
		'SELECT grandparent.parentid FROM ^categories JOIN ^categories AS parent ON ^categories.parentid=parent.categoryid JOIN ^categories AS grandparent ON parent.parentid=grandparent.categoryid', // 2 gens up
		'SELECT parent.parentid FROM ^categories JOIN ^categories AS parent ON ^categories.parentid=parent.categoryid',
		// 1 gen up
		'SELECT parentid FROM ^categories', // same gen
		'SELECT categoryid FROM ^categories', // gen below
	);
	...
	$selectspec = array(
		'columns' => $columns,
		'source' => '^categories JOIN (' . implode(' UNION ', $parentselects) . ') y ON ^categories.parentid<=>parentkey' .
			' LEFT JOIN ^categories AS child ON child.parentid=^categories.categoryid GROUP BY ^categories.categoryid' .
...
/**
 * Return the selectspec to retrieve the most recent items with $tag, with the corresponding like on those
 * items made by $likeuserid (if not null) and including $full content or not. Return $count (if null, a default is
 * used) items starting from $start.
 * @param $likeuserid
 * @param $tag
 * @param $start
 * @param bool $full
 ...
 * @return array
 */
function as_db_tag_recent_qs_selectspec($likeuserid, $tag, $start, $full = false, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
...
/**
 * Return the selectspec to retrieve the number of items tagged with $tag (single value)
 * @param $tag
 * @return array
 */
...
/**
 * Return the selectspec to retrieve recent items by the user identified by $identifier, where $identifier is a
 ...
 * @return array
 */
function as_db_user_recent_qs_selectspec($likeuserid, $identifier, $count = null, $start = 0)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;

	$selectspec = as_db_posts_basic_selectspec($likeuserid);

	$selectspec['source'] .= " WHERE ^posts.userid=" . (AS_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") . " AND ^posts.type='P' ORDER BY ^posts.created DESC LIMIT #,#";
	array_push($selectspec['arguments'], $identifier, $start, $count);
	$selectspec['sortdesc'] = 'created';
...
/**
 * Return the selectspec to retrieve the antecedent items for recent reviews by the user identified by $identifier
 * (see as_db_user_recent_qs_selectspec() comment), with the corresponding like on those items made by $likeuserid
 * (if not null). Return $count (if null, a default is used) items. The selectspec will also retrieve some
 ...
 * @return array
 */
function as_db_user_recent_a_qs_selectspec($likeuserid, $identifier, $count = null, $start = 0)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
	...
	$selectspec['columns']['opositivelikes'] = 'aposts.positivelikes';
	$selectspec['columns']['onegativelikes'] = 'aposts.negativelikes';
	$selectspec['columns']['onetlikes'] = 'aposts.netlikes';

	$selectspec['source'] .=
		" JOIN ^posts AS aposts ON ^posts.postid=aposts.parentid" .
		" JOIN (SELECT postid FROM ^posts WHERE " .
		" userid=" . (AS_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") .
		" AND type='R' ORDER BY created DESC LIMIT #,#) y ON aposts.postid=y.postid WHERE ^posts.type='P'";
...
/**
 * Return the selectspec to retrieve the antecedent items for recent comments by the user identified by $identifier
 * (see as_db_user_recent_qs_selectspec() comment), with the corresponding like on those items made by $likeuserid
 * (if not null). Return $count (if null, a default is used) items. The selectspec will also retrieve some
 ...
 * @return array
 */
function as_db_user_recent_c_qs_selectspec($likeuserid, $identifier, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
	...
	$selectspec['source'] .=
		...
		" JOIN (SELECT postid FROM ^posts WHERE " .
		" userid=" . (AS_FINAL_EXTERNAL_USERS ? "$" : "(SELECT userid FROM ^users WHERE handle=$ LIMIT 1)") .
		" AND type='C' ORDER BY created DESC LIMIT #) y ON cposts.postid=y.postid WHERE ^posts.type='P' AND parentposts.type IN ('P', 'R')";
...
/**
 * Return the selectspec to retrieve the antecedent items for recently edited posts by the user identified by
 * $identifier (see as_db_user_recent_qs_selectspec() comment), with the corresponding like on those items made by
 * $likeuserid (if not null). Return $count (if null, a default is used) items. The selectspec will also retrieve
 ...
 * @return array
 */
function as_db_user_recent_edit_qs_selectspec($likeuserid, $identifier, $count = null)
{
	$count = isset($count) ? min($count, AS_DB_RETRIEVE_QS_AS) : AS_DB_RETRIEVE_QS_AS;
	...
	$selectspec['source'] .=
		...
		" AND type IN ('P', 'R', 'C') ORDER BY updated DESC LIMIT #) y ON editposts.postid=y.postid " .
		" WHERE parentposts.type IN ('P', 'R', 'C') AND ^posts.type IN ('P', 'R', 'C')";
...
{
	return array(
		'columns' => array('points', 'qposts', 'aposts', 'cposts', 'aselects', 'aselecteds', 'qpositivelikes', 'qnegativelikes', 'apositivelikes', 'anegativelikes', 'qlikeds', 'alikeds', 'positivelikeds', 'negativelikeds', 'bonus'),
...
function as_db_recent_messages_selectspec($fromidentifier, $fromisuserid, $toidentifier, $toisuserid, $count = null, $start = 0)
{
	...
	if (isset($fromidentifier)) {
		...
		$where = 'fromuserid=' . $fromsub . " AND ^messages.type='PRIVATE'";
	} else {
		$where = "^messages.type='PUBLIC'";
	}
 ...
 * The $identifier should be a handle, word, backpath or postid for users, tags, categories and items respectively.
 * @param $userid
 * @param $entitytype
 * @param $identifier
 * @return array
...
/**
 * Return the selectspec to retrieve an array of $userid's favorited items, with the usual information.
 * Returns $limit items, or all of them if $limit is null (used in as_db_selectspec_count).
 * @param $userid
 * @param $limit
 * @param int $start
 * @return array
...
function as_db_user_favorite_categories_selectspec($userid)
{
	...
	return array(
		'columns' => array('categoryid', 'title', 'tags', 'pcount', 'backpath', 'content'),
...
/**
 * Return the selectspec to retrieve information about all a user's favorited items except the items. Depending on
...
function as_db_user_favorite_non_qs_selectspec($userid)
{
	...
	return array(
		...
		'arguments' => array(AS_ENTITY_USER, AS_ENTITY_TAG, AS_ENTITY_CATEGORY, $userid, AS_ENTITY_ARTICLE),
	);
}
...
function as_db_user_updates_selectspec($userid, $forfavorites = true, $forcontent = true)
{
	...
	if ($forfavorites) { // life is hard
		$selectspec['source'] .= ' JOIN ' .
			"(SELECT entitytype, articleid, lastpostid, updatetype, lastuserid, updated FROM ^userevents WHERE userid=$" .
			($forcontent ? '' : " AND entitytype!=" . $nonesql) .
			" UNION SELECT ^sharedevents.entitytype, articleid, lastpostid, updatetype, lastuserid, updated FROM ^sharedevents JOIN ^userfavorites ON ^sharedevents.entitytype=^userfavorites.entitytype AND ^sharedevents.entityid=^userfavorites.entityid AND ^userfavorites.nouserevents=1 WHERE userid=$) fullevents ON ^posts.postid=fullevents.articleid";

		array_push($selectspec['arguments'], $userid, $userid);

	} else { // life is easy
		$selectspec['source'] .= " JOIN ^userevents AS fullevents ON ^posts.postid=fullevents.articleid AND fullevents.userid=$ AND fullevents.entitytype=" . $nonesql;
		$selectspec['arguments'][] = $userid;
	}

	$selectspec['source'] .=
		" JOIN ^posts AS updateposts ON updateposts.postid=fullevents.lastpostid" .
		" AND (updateposts.type IN ('P', 'R', 'C') OR fullevents.entitytype=" . $nonesql . ")" .
		" AND (^posts.selchildid=fullevents.lastpostid OR NOT fullevents.updatetype<=>$) AND ^posts.type IN ('P', 'P_HIDDEN')" .
	...
	unset($selectspec['arraykey']); // allow same item to be retrieved multiple times
