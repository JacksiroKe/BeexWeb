	...
	*/
	...
	$tables = array(
		'users' => array(
			...
			'type' => "ENUM('SA', 'A', 'ENR', 'ER', 'SSP', 'MSP', 'BC', 'IC', 'BS', 'IS', 'U') NOT NULL DEFAULT 'U'",
			...
			'gender' => 'TINYINT UNSIGNED NOT NULL', // is male, female, organisation...
			...
			'profiles' => 'VARCHAR(' . AS_DB_MAX_PROFILE_TITLE_LENGTH . ') NOT NULL', // profiles
		...
		'suppliers' => array(
			'supplierid' => $useridcoltype . ' NOT NULL AUTO_INCREMENT',
		...
		'employees' => array(
			'employeeid' => $useridcoltype . ' NOT NULL AUTO_INCREMENT',
			'userid' => $useridcoltype . ' NOT NULL',
			'title' => 'VARCHAR(' . AS_DB_MAX_PROFILE_TITLE_LENGTH . ') NOT NULL',
			'content' => 'VARCHAR(' . AS_DB_MAX_PROFILE_CONTENT_LENGTH . ') NOT NULL',
			'location' => 'VARCHAR(' . AS_DB_MAX_PROFILE_TITLE_LENGTH . ') NOT NULL',
		...
		'userevents' => array(
			...
			'articleid' => 'INT UNSIGNED NOT NULL', // the affected item
			'lastpostid' => 'INT UNSIGNED NOT NULL', // what part of item was affected
			...
			'KEY articleid (articleid, userid)', // to limit number of events per item per stream
		),

		'sharedevents' => array(
			'entitytype' => 'CHAR(1) CHARACTER SET ascii NOT NULL', // see /as-include/app/updates.php
			'entityid' => 'INT UNSIGNED NOT NULL', // see userfavorites table
			'articleid' => 'INT UNSIGNED NOT NULL', // see userevents table
			...
			'KEY articleid (articleid, entitytype, entityid)', // to limit number of events per item per stream
		),
		...
		'categories' => array(
			...
			'pcount' => 'INT UNSIGNED NOT NULL DEFAULT 0',
		...
		'posts' => array(
			...
			'type' => "ENUM('P', 'R', 'C', 'P_HIDDEN', 'R_HIDDEN', 'C_HIDDEN', 'P_QUEUED', 'R_QUEUED', 'C_QUEUED', 'NOTE') NOT NULL",
			'categoryid' => 'INT UNSIGNED', // this is the canonical final category id
			'title' => 'VARCHAR(' . AS_DB_MAX_TITLE_LENGTH . ')',
			'itemcode' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL DEFAULT \'\'', // item code
			'volume' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL DEFAULT \'\'', // item volume
			'mass' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL DEFAULT \'\'', // item mass
			'texture' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL DEFAULT \'\'', // item mass
			'content' => 'VARCHAR(' . AS_DB_MAX_CONTENT_LENGTH . ')',
			...
			'lastuserid' => $useridcoltype, // which user last modified it
			'lastip' => 'VARBINARY(16)', // INET6_ATON of IP address which last modified the post
			'positivelikes' => 'SMALLINT UNSIGNED NOT NULL DEFAULT 0',
			'negativelikes' => 'SMALLINT UNSIGNED NOT NULL DEFAULT 0',
			'netlikes' => 'SMALLINT NOT NULL DEFAULT 0',
			...
			'PRIMARY KEY (postid)',
			'KEY type (type, created)', // for getting recent items, reviews, comments
			'KEY type_2 (type, rcount, created)', // for getting unreviewed items
			'KEY type_4 (type, netlikes, created)', // for getting posts with the most likes
			'KEY type_5 (type, views, created)', // for getting items with the most views
			'KEY type_6 (type, hotness)', // for getting 'hot' items
			'KEY type_7 (type, rmaxlike, created)', // for getting items with no positiveliked reviews
			'KEY parentid (parentid, type)', // for getting a item's reviews, any post's comments and follow-on items
			'KEY userid (userid, type, created)', // for recent items, reviews or comments by a user
			...
			'KEY catidpath1 (catidpath1, type, created)', // for getting item, reviews or comments in a specific level category
		...
		'orders' => array(  
			...
			'lastip' => 'VARBINARY(16)', // INET6_ATON of IP address which last modified the post
			'lastviewip' => 'VARBINARY(16)', // INET6_ATON of IP address which last viewed the post
			'address' => 'VARCHAR(' . AS_DB_MAX_OTHER_LENGTH . ')',
			'extra' => 'VARCHAR(' . AS_DB_MAX_CONTENT_LENGTH . ')',
			'created' => 'DATETIME NOT NULL',
			'updated' => 'DATETIME',
			'PRIMARY KEY (orderid)',
		),
		
		'customers' => array( //type, business, userid, title, idnumber, content, icon, phone, email, location1, location2, location3, location4, created
			...
			'phone' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL',
			'email' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ') NOT NULL',
			...
			'field1' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ')',
			'field2' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ')',
			'field3' => 'VARCHAR(' . AS_DB_MAX_CAT_PAGE_TITLE_LENGTH . ')',
			'created' => 'DATETIME NOT NULL',
			'updated' => 'DATETIME',
		...
		'stock' => array( //type, business, itemid, userid, quantity, created
			...
			'actual' => 'INT UNSIGNED NOT NULL DEFAULT 0',
		...
		'contentwords' => array(
			...
			'type' => "ENUM('P', 'R', 'C', 'NOTE') NOT NULL", // the post's type (copied here for quick searching)
		...
		'posttags' => array(
			'postid' => 'INT UNSIGNED NOT NULL',
			'wordid' => 'INT UNSIGNED NOT NULL',
			'postcreated' => 'DATETIME NOT NULL', // created time of post (copied here for tag page's list of recent items)
			'KEY postid (postid)',
		...
		'userlikes' => array(
			'postid' => 'INT UNSIGNED NOT NULL',
			'userid' => $useridcoltype . ' NOT NULL',
			'lyke' => 'TINYINT NOT NULL', // -1, 0 or 1
			'flag' => 'TINYINT NOT NULL', // 0 or 1
			'likecreated' => 'DATETIME', // time of first like
			'likeupdated' => 'DATETIME', // time of last like change
			'UNIQUE userid (userid, postid)',
			'KEY postid (postid)',
			'KEY liked (likecreated, likeupdated)',
			'CONSTRAINT ^userlikes_ibfk_1 FOREIGN KEY (postid) REFERENCES ^posts(postid) ON DELETE CASCADE',
		),
		...
		'userpoints' => array(
			...
			'qposts' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of items by user (excluding hidden/queued)
			...
			'aselects' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of items by user where they've selected an review
			'aselecteds' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of reviews by user that have been selected as the best
			'qpositivelikes' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of items the user has liked up
			'qnegativelikes' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of items the user has liked down
			'apositivelikes' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of reviews the user has liked up
			'anegativelikes' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of reviews the user has liked down
			'cpositivelikes' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of comments the user has liked up
			'cnegativelikes' => 'MEDIUMINT NOT NULL DEFAULT 0', // number of comments the user has liked down
			'qlikeds' => 'INT NOT NULL DEFAULT 0', // points from likes on this user's items (applying per-item limits), before final multiple
			'alikeds' => 'INT NOT NULL DEFAULT 0', // points from likes on this user's reviews (applying per-review limits), before final multiple
			'clikeds' => 'INT NOT NULL DEFAULT 0', // points from likes on this user's comments (applying per-comment limits), before final multiple
			'positivelikeds' => 'INT NOT NULL DEFAULT 0', // number of up likes received on this user's items or reviews
			'negativelikeds' => 'INT NOT NULL DEFAULT 0', // number of down likes received on this user's items or reviews
	...
	if (AS_FINAL_EXTERNAL_USERS) {
		...
		$tables['userlikes'][] = 'CONSTRAINT ^userlikes_ibfk_2 ' . $userforeignkey . ' ON DELETE CASCADE';
	...
	if (count($missing) == count($definitions))
		...
		if (count($missing)) {
			if (defined('AS_MYSQL_USERS_PREFIX')) { // special case if two installations sharing users
				...
				if ($datrcount == $datamissing && $datamissing == count($missing))
...
function as_db_upgrade_tables()
{
	...
	while (1) {
		...
		switch ($newversion) {
			...
			case 2:
				as_db_upgrade_query('ALTER TABLE ^posts DROP COLUMN likes, ADD COLUMN positivelikes ' . $definitions['posts']['positivelikes'] .
					' AFTER cookieid, ADD COLUMN negativelikes ' . $definitions['posts']['negativelikes'] . ' AFTER positivelikes');
				as_db_upgrade_query($locktablesquery);
				$keyrecalc['dorecountposts'] = true;
				break;

			case 3:
				as_db_upgrade_query('ALTER TABLE ^userpoints ADD COLUMN positivelikeds ' . $definitions['userpoints']['positivelikeds'] .
					' AFTER alikeds, ADD COLUMN negativelikeds ' . $definitions['userpoints']['negativelikeds'] . ' AFTER positivelikeds');
				as_db_upgrade_query($locktablesquery);
			...
			case 8:
				as_db_upgrade_query('ALTER TABLE ^posts ADD KEY (type, rcount, created)');
				as_db_upgrade_query($locktablesquery);
				$keyrecalc['dorecountposts'] = true; // for unreviewed item count
				break;
			...
			case 10:
				...
				as_db_upgrade_query(as_db_create_table_sql('categories', array(
					...
					'pcount' => $definitions['categories']['pcount'],
			...
			case 14:
				as_db_upgrade_query('ALTER TABLE ^userpoints DROP COLUMN qlikes, DROP COLUMN alikes, ADD COLUMN qpositivelikes ' . $definitions['userpoints']['qpositivelikes'] . ' AFTER aselecteds, ADD COLUMN qnegativelikes ' . $definitions['userpoints']['qnegativelikes'] . ' AFTER qpositivelikes, ADD COLUMN apositivelikes ' . $definitions['userpoints']['apositivelikes'] . ' AFTER qnegativelikes, ADD COLUMN anegativelikes ' . $definitions['userpoints']['anegativelikes'] . ' AFTER apositivelikes');
				as_db_upgrade_query($locktablesquery);
				$keyrecalc['dorecalcpoints'] = true;
				break;
			...
			case 15:
				...
				as_db_upgrade_table_columns($definitions, 'posts', array('rcount', 'positivelikes', 'negativelikes', 'format'));
				as_db_upgrade_table_columns($definitions, 'categories', array('pcount'));
				...
				as_db_upgrade_table_columns($definitions, 'userpoints', array('points', 'qposts', 'aposts', 'cposts',
					'aselects', 'aselecteds', 'qpositivelikes', 'qnegativelikes', 'apositivelikes', 'anegativelikes', 'qlikeds', 'alikeds', 'positivelikeds', 'negativelikeds'));
				as_db_upgrade_query($locktablesquery);
				break;
			...
			case 26:
				as_db_upgrade_query('ALTER TABLE ^userlikes ADD COLUMN flag ' . $definitions['userlikes']['flag'] . ' AFTER lyke');
				as_db_upgrade_query($locktablesquery);

				as_db_upgrade_query('ALTER TABLE ^posts ADD COLUMN flagcount ' . $definitions['posts']['flagcount'] . ' AFTER negativelikes, ADD KEY type_3 (type, flagcount, created)');
				as_db_upgrade_query($locktablesquery);
			...
			case 27:
				as_db_upgrade_query('ALTER TABLE ^posts ADD COLUMN netlikes ' . $definitions['posts']['netlikes'] . ' AFTER negativelikes, ADD KEY type_4 (type, netlikes, created)');
				as_db_upgrade_query($locktablesquery);
			...
			case 28:
				as_db_upgrade_query('ALTER TABLE ^posts ADD COLUMN views ' . $definitions['posts']['views'] . ' AFTER netlikes, ADD COLUMN hotness ' . $definitions['posts']['hotness'] . ' AFTER views, ADD KEY type_5 (type, views, created), ADD KEY type_6 (type, hotness)');
				as_db_upgrade_query($locktablesquery);
			...
			case 29:
				as_db_upgrade_query('ALTER TABLE ^posts ADD COLUMN lastviewip ' . $definitions['posts']['lastviewip'] . ' AFTER netlikes');
				as_db_upgrade_query($locktablesquery);
				break;
			...
			case 36:
				...
				as_db_upgrade_query(as_db_create_table_sql('userevents', array(
					...
					'articleid' => $definitions['userevents']['articleid'],
			...
			case 40:
				as_db_upgrade_query('ALTER TABLE ^userpoints ADD COLUMN bonus ' . $definitions['userpoints']['bonus'] . ' AFTER negativelikeds');
				as_db_upgrade_query($locktablesquery);
				break;
			...
			case 46:
				as_db_upgrade_query('ALTER TABLE ^posts DROP KEY selchildid, ADD KEY selchildid (selchildid, type, created), ADD COLUMN amaxlike SMALLINT UNSIGNED NOT NULL DEFAULT 0 AFTER rcount, ADD KEY type_7 (type, amaxlike, created)');
				as_db_upgrade_query($locktablesquery);
			...
			case 58:
				...
				as_db_upgrade_query('DELETE ^userlikes FROM ^userlikes JOIN ^posts ON ^userlikes.postid=^posts.postid AND ^userlikes.userid=^posts.userid');
				as_db_upgrade_query($locktablesquery);
			...
			case 60:
				// add new category widget - note title must match that from as_register_core_modules()
			...
			case 65:
				as_db_upgrade_query('ALTER TABLE ^userlikes ADD COLUMN likecreated ' . $definitions['userlikes']['likecreated'] . ' AFTER flag');
				as_db_upgrade_query('ALTER TABLE ^userlikes ADD COLUMN likeupdated ' . $definitions['userlikes']['likeupdated'] . ' AFTER likecreated');
				as_db_upgrade_query('ALTER TABLE ^userlikes ADD KEY liked (likecreated, likeupdated)');
				as_db_upgrade_query($locktablesquery);

				// for old likes, set a default date of when that post was made
				as_db_upgrade_query('UPDATE ^userlikes, ^posts SET ^userlikes.likecreated=^posts.created WHERE ^userlikes.postid=^posts.postid AND (^userlikes.lyke != 0 OR ^userlikes.flag=0)');
				break;

			case 66:
				$newColumns = array(
					'ADD COLUMN cpositivelikes ' . $definitions['userpoints']['cpositivelikes'] . ' AFTER anegativelikes',
					'ADD COLUMN cnegativelikes ' . $definitions['userpoints']['cnegativelikes'] . ' AFTER cpositivelikes',
					'ADD COLUMN clikeds ' . $definitions['userpoints']['clikeds'] . ' AFTER alikeds',
				);
