...
/**
 * Return the stock in a particular category
 */
function as_db_category_stock($categoryid)
{
	return as_db_read_one_value(as_db_query_sub(
		"SELECT stock FROM ^categories WHERE categoryid=#",
		$categoryid
	));
}
	
function as_db_stock_count_update($categoryid, $quantity)
{
	$oldstock = as_db_category_stock($categoryid);	
	as_db_query_sub(
		'UPDATE ^categories SET stock=' . ($oldstock . $quantity) . ' WHERE categoryid=#',
		$categoryid
	);
}

function as_db_item_stock($postid)
{
	return as_db_read_one_value(as_db_query_sub(
		"SELECT quantity FROM ^posts WHERE postid=#",
		$postid
	));
}
	
function as_db_item_count_update($postid, $quantity)
{
	$oldstock = as_db_item_stock($postid);	
	as_db_query_sub(
		'UPDATE ^posts SET quantity=' . ($oldstock . $quantity) . ' WHERE postid=#',
		$postid
	);
}
/**
...
{
	return as_db_read_all_assoc(as_db_query_sub(
		"SELECT ^posts.postid, ^posts.title, ^posts.content, ^posts.format, ^posts.tags, ^posts.categoryid, ^posts.type, IF (^posts.type='P', ^posts.postid, IF(parent.type='P', parent.postid, grandparent.postid)) AS articleid, ^posts.parentid FROM ^posts LEFT JOIN ^posts AS parent ON ^posts.parentid=parent.postid LEFT JOIN ^posts as grandparent ON parent.parentid=grandparent.postid WHERE ^posts.postid>=# AND ( (^posts.type='P') OR (^posts.type='R' AND parent.type<=>'P') OR (^posts.type='C' AND parent.type<=>'P') OR (^posts.type='C' AND parent.type<=>'R' AND grandparent.type<=>'P') ) ORDER BY postid LIMIT #",
		$startpostid, $count
	), 'postid');
}
...
// For recalculating numbers of likes and reviews for items...
...
/**
 * Recalculate the cached like counts for posts $firstpostid to $lastpostid in the database
 * @param $firstpostid
 * @param $lastpostid
 */
function as_db_posts_likes_recount($firstpostid, $lastpostid)
{
	as_db_query_sub(
		'UPDATE ^posts AS x, (SELECT ^posts.postid, COALESCE(SUM(GREATEST(0,^userlikes.lyke)),0) AS positivelikes, -COALESCE(SUM(LEAST(0,^userlikes.lyke)),0) AS negativelikes, COALESCE(SUM(IF(^userlikes.flag, 1, 0)),0) AS flagcount FROM ^posts LEFT JOIN ^userlikes ON ^userlikes.postid=^posts.postid WHERE ^posts.postid>=# AND ^posts.postid<=# GROUP BY postid) AS a SET x.positivelikes=a.positivelikes, x.negativelikes=a.negativelikes, x.netlikes=a.positivelikes-a.negativelikes, x.flagcount=a.flagcount WHERE x.postid=a.postid',
		$firstpostid, $lastpostid
	);
...
/**
 * Recalculate the cached review counts for posts $firstpostid to $lastpostid in the database, along with the highest netlikes of any of their reviews
 * @param $firstpostid
 * @param $lastpostid
 */
function as_db_posts_reviews_recount($firstpostid, $lastpostid)
{
	...
	as_db_query_sub(
		'UPDATE ^posts AS x, (SELECT parents.postid, COUNT(children.postid) AS rcount, COALESCE(GREATEST(MAX(children.netlikes), 0), 0) AS rmaxlike FROM ^posts AS parents LEFT JOIN ^posts AS children ON parents.postid=children.parentid AND children.type=\'A\' WHERE parents.postid>=# AND parents.postid<=# GROUP BY postid) AS a SET x.rcount=a.rcount, x.rmaxlike=a.rmaxlike WHERE x.postid=a.postid',
		$firstpostid, $lastpostid
	);
...
function as_db_users_get_for_recalc_points($startuserid, $count)
{
	if (AS_FINAL_EXTERNAL_USERS) {
		return as_db_read_all_values(as_db_query_sub(
			'SELECT userid FROM ((SELECT DISTINCT userid FROM ^posts WHERE userid>=# ORDER BY userid LIMIT #) UNION (SELECT DISTINCT userid FROM ^userlikes WHERE userid>=# ORDER BY userid LIMIT #)) x ORDER BY userid LIMIT #',
...
function as_db_users_recalc_points($firstuserid, $lastuserid)
{
	...
	if (AS_FINAL_EXTERNAL_USERS) {
		as_db_query_sub(
			'INSERT IGNORE INTO ^userpoints (userid) SELECT DISTINCT userid FROM ^posts WHERE userid>=# AND userid<=# UNION SELECT DISTINCT userid FROM ^userlikes WHERE userid>=# AND userid<=#',
...
/**
 * Return the ids of up to $count items in the database starting from $startpostid
 * @param $startpostid
 * @param $count
 * @return array
 */
...
{
	return as_db_read_all_values(as_db_query_sub(
		"SELECT postid FROM ^posts WHERE postid>=# AND LEFT(type, 1)='P' ORDER BY postid LIMIT #",
		$startpostid, $count
	));
}
 ...
 * in the database, where the category of comments and reviews is set by the category of the antecedent item
 * @param $firstpostid
 * @param $lastpostid
 */
...
{
	as_db_query_sub(
		"UPDATE ^posts AS x, (SELECT ^posts.postid, IF(LEFT(parent.type, 1)='P', parent.categoryid, grandparent.categoryid) AS categoryid FROM ^posts LEFT JOIN ^posts AS parent ON ^posts.parentid=parent.postid LEFT JOIN ^posts AS grandparent ON parent.parentid=grandparent.postid WHERE ^posts.postid BETWEEN # AND # AND LEFT(^posts.type, 1)!='P') AS a SET x.categoryid=a.categoryid WHERE x.postid=a.postid",
		$firstpostid, $lastpostid
	);
}
