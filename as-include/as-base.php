...
function as_initialize_constants_1()
{
	...
	define('AS_MEDIA_DIR', AS_BASE_DIR . 'as-media/');
...
/**
 * signup all modules that come as part of the APS core (as opposed to plugins)
 */
function as_register_core_modules()
{
	as_register_module('filter', 'plugins/as-filter-basic.php', 'as_filter_basic', '');
	as_register_module('editor', 'plugins/as-editor-basic.php', 'as_editor_basic', '');
	as_register_module('viewer', 'plugins/as-viewer-basic.php', 'as_viewer_basic', '');
	as_register_module('event', 'plugins/as-event-limits.php', 'as_event_limits', 'APS Event Limits');
	as_register_module('event', 'plugins/as-event-notify.php', 'as_event_notify', 'APS Event Notify');
	as_register_module('event', 'plugins/as-event-updates.php', 'as_event_updates', 'APS Event Updates');
	as_register_module('search', 'plugins/as-search-basic.php', 'as_search_basic', '');
	as_register_module('widget', 'plugins/as-widget-activity-count.php', 'as_activity_count', 'Activity Count');
	as_register_module('widget', 'plugins/as-widget-write-box.php', 'as_write_box', 'Post Box');
	as_register_module('widget', 'plugins/as-widget-related-qs.php', 'as_related_qs', 'Related Articles');
	as_register_module('widget', 'plugins/as-widget-category-list.php', 'as_category_list', 'Categories');
}
...
{
	$fields = array(
		'min_aps' => 'Minimum AppSmata Version',
...
/**
 * signup a module of $type named $name, whose class named $class is defined in file $include (or null if no include necessary)
...
/**
 * signup a layer named $name, defined in file $include. If this layer comes from a plugin (as all currently do),
...
/**
 * signup a file $include containing override functions. If this file comes from a plugin (as all currently do),
 * pass in the local plugin $directory and the $urltoroot relative url for that directory
 * @param $include
 * @param string $directory
 * @param string $urltoroot
...
/**
 * signup a set of language phrases, which should be accessed by the prefix $name/ in the as_lang_*() functions.
...
/**
 * signup a plugin module of $type named $name, whose class named $class is defined in file $include (or null if no include necessary)
...
/**
 * signup a plugin layer named $name, defined in file $include. Can only be called from a plugin's as-plugin.php file
 * @param $include
 * @param $name
 */
...
/**
 * signup a plugin file $include containing override functions. Can only be called from a plugin's as-plugin.php file
 * @param $include
 */
...
/**
 * signup a file name $pattern within a plugin directory containing language phrases accessed by the prefix $name
 * @param $pattern
 * @param $name
 */
...
function as_eval_from_file($eval, $filename)
{
	...
	if (strlen($php_errormsg)) {
		...
		@error_log('PHP AppSmata more info: ' . $php_errormsg . " in eval()'d code from " . as_html($filename));
	}
...
function as_fatal_error($message)
{
	...
	echo 'AppSmata fatal error:<p style="color: red">' . as_html($message, true) . '</p>';
...
function as_path($request, $params = null, $rooturl = null, $neaturls = null, $anchor = null)
{
		...
		case AS_URL_FORMAT_PARAMS:
			if (!empty($request)) {
				foreach ($requestparts as $partindex => $requestpart)
					$paramgendertra .= (strlen($paramgendertra) ? '&' : '?') . 'as' . ($partindex ? ('_' . $partindex) : '') . '=' . $requestpart;
			}
			break;
	}
	...
	if (is_array($params)) {
		foreach ($params as $key => $value) {
			...
			$paramgendertra .= (strlen($paramgendertra) ? '&' : '?') . urlencode($key) . '=' . urlencode($value);
		}
	}

	return $url . $paramgendertra . (empty($anchor) ? '' : '#' . urlencode($anchor));
}
...
/**
 * Get APS request for a item, and make it search-engine friendly, shortening it if necessary
 * by removing shorter words which are generally less meaningful.
 * @param int $articleid The item ID
 * @param string $title The item title
 * @return string
 */
function as_q_request($articleid, $title)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }
...
/**
 * Return the URL for item $articleid with $title, possibly using $absolute URLs.
 * To link to a specific review or comment in a item, set $showtype and $showid accordingly.
 * @param $articleid
 * @param $title
 * @param bool $absolute
 ...
 * @return mixed|string
 */
function as_q_path($articleid, $title, $absolute = false, $showtype = null, $showid = null)
{
	if (as_to_override(__FUNCTION__)) { $args=func_get_args(); return as_call_override(__FUNCTION__, $args); }

	if (($showtype == 'P' || $showtype == 'R' || $showtype == 'C') && isset($showid)) {
	...
	return as_path(as_q_request($articleid, $title), $params, $absolute ? as_opt('site_url') : null, null, $anchor);
}


/**
 * Return the HTML representation of the URL for $articleid - other parameters as for as_q_path()
 * @param $articleid
 * @param $title
 * @param bool $absolute
 * @param string $showtype
 ...
 * @return mixed|string
 */
function as_q_path_html($articleid, $title, $absolute = false, $showtype = null, $showid = null)
{
	return as_html(as_q_path($articleid, $title, $absolute, $showtype, $showid));
}
...
function as_path_form_html($request, $params = null, $rooturl = null, $neaturls = null, $anchor = null)
{
	...
	$articlepos = strpos($path, '?');
	if (is_numeric($articlepos)) {
		$params = explode('&', substr($path, $articlepos + 1));
...
function as_upload_file($upload, $default, $name=null){
	...
	$finalname = ($name ? $name : 'post').'_'.time().'.'.$upload_file_ext[1];
...
function as_upload_file_mobile($upload, $default, $name=null){
	$raw_file_name = $upload["name"];
	$temp_file_name = $upload["tmp_name"];		
	$upload_file_ext = explode(".", $raw_file_name);
	$upload_file_name = preg_replace("/-$/","",preg_replace('/[^a-z0-9]+/i', "_", strtolower($upload_file_ext[0])));
