 ...
 * relative path from the root of the APS site to your login page. Like in example 2 below, if
...
function as_get_login_links($relative_url_prefix, $redirect_back_to_url)
{
	...
	*/
	...
	/*
		...
		return array(
			'login' => $relative_url_prefix.'../login.php?redirect='.urlencode('as/'.$redirect_back_to_url),
			'register' => $relative_url_prefix.'../register.php?redirect='.urlencode('as/'.$redirect_back_to_url),
			'logout' => $relative_url_prefix.'../logout.php?redirect='.urlencode('as/'.$redirect_back_to_url),
		);
	*/
}
 ...
 * logout link on the APS pages. This HTML will only be shown to the logged in user themselves.
